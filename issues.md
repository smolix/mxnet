...
| XOP5 | Open | Shape inference | GroupNorm and InstanceNorm shape inference silently overwrites caller-provided bad gamma/beta shapes, unlike LayerNorm's `SHAPE_ASSIGN_CHECK`. | Use checked shape assignment for provided gamma/beta shapes and add symbolic shape-inference regressions. |
...
| XOP8 | Open | Quantized range outputs | Multiple DNNL quantized ops overwrite scalar min/max outputs without honoring `req`: quantized reshape, flatten, activation, BatchNorm, elemwise add, concat, quantized batch-dot, requantize, quantize_v2, and related range-output paths. Quantized transpose has the better req-aware pattern. | Apply the quantized-transpose range-output helper broadly and add parameterized `kNullOp`/`kAddTo` tests for range outputs. |
