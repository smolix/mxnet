...
| GH3 | In progress | Autograd/Gluon semantics | Unresolved gradient/export semantics include `grad_req='add'`, non-leaf `attach_grad`, `autograd.grad`, `block.export`, mixed binary backward, and dtype propagation. `autograd.grad(heads, variable)` now preserves the documented single-NDArray return shape instead of returning a one-element list after internally normalizing the variable argument. `autograd.grad()` also wraps returned gradient handles to match each variable's array class, so NumPy-mode variables receive `mx.np.ndarray` gradients instead of legacy `mx.nd.NDArray`. | Continue with the remaining semantics items, starting with CPU-reproducible tests before adding CUDA parity. |
...
| GH6 | In progress | Data/datasets | Dataset/RecordIO issues remain: multithreaded RecordIO file-id switching, ImageFolderDataset classes, transform handling, and MultiboxPrior coverage. The C++ `RecordFileDataset` handle now resets its thread-local RecordIO reader when switching between `.rec` files, and `ImageFolderDataset` accepts an explicit `classes=` order for split-stable labels while rejecting invalid class lists. | Continue with transform handling and MultiboxPrior coverage after committing the validated RecordIO/ImageFolder slice. |
