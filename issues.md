...
| GH3 | In progress | Autograd/Gluon semantics | Unresolved gradient/export semantics include `grad_req='add'`, non-leaf `attach_grad`, `autograd.grad`, `block.export`, mixed binary backward, and dtype propagation. `autograd.grad(heads, variable)` now preserves the documented single-NDArray return shape instead of returning a one-element list after internally normalizing the variable argument. | Continue with the remaining semantics items, starting with CPU-reproducible tests before adding CUDA parity. |
