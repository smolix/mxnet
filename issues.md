...
| ID | Status | Area | Issue | Next action |
|---|---|---|---|---|
| GH1 | In progress | Security/tooling | Open upstream security/tooling hardening includes unsafe extraction, command injection risk in notebook conversion, insecure links, and stale docs/CD dependencies. Dependency zip extraction for the local OpenCV and libturbojpeg builders now rejects archive members that escape the intended extraction directory, with focused tests covering allowed contents and path traversal. | Continue targeted hardening: `tools/ipynb2md.py` still uses `os.system` with user-controlled paths, `tools/kill-mxnet.py` still has command-execution surface, older-Python `build_openmp.py` tar extraction fallback is unsafe, and dependency downloads still lack checksum/signature verification. |
