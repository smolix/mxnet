...
|---|---|---|---|---|
| XOP1 | Open | Norm state semantics | Native CUDA BatchNorm with `cudnn_off=True` still updates moving stats in backward, not forward; SyncBatchNorm has the same design in `src/operator/contrib/sync_batch_norm-inl.h`. Forward-only training, stopped gradients, or failed backward can leave aux state stale. | Move running-stat updates into forward for native CUDA BatchNorm and SyncBatchNorm, then add forward-only and backward-failure regressions. |
| XOP2 | Open | Norm affine semantics | BatchNorm and SyncBatchNorm mutate gamma for `fix_gamma=True` even though mutation metadata does not declare gamma; `fix_gamma=True` also mishandles `grad_req='add'` for gamma on native CUDA/cuDNN paths. | Stop mutating gamma for fixed-gamma paths, preserve add/null grad semantics, and restore tests that currently skip gamma grad checks. |
| XOP3 | Open | oneDNN LayerNorm | oneDNN LayerNorm exposes variance in MXNet's `std` output and ignores true `kAddTo` accumulation for gamma/beta grads. Its mean/std outputs also bypass req-aware output allocation. | Convert oneDNN variance to std for visible outputs or adjust backward inputs carefully; implement explicit add-back for gamma/beta; add visible-output and `grad_req='add'` tests using DNNL-sized tensors. |
| XOP4 | Open | fp16 reductions | Generic non-last-axis LayerNorm and GroupNorm reduce into fp16 visible moment buffers before division/sqrt; large reductions overflow to `inf`/`nan` even with `MXNET_SAFE_ACCUMULATION=1`. | Accumulate mean/variance in fp32 or divide during accumulation for fp16; add large fp16 CPU/GPU tests for LayerNorm, GroupNorm, and adjacent reduction-normalization paths. |
| XOP5 | Open | Shape inference | GroupNorm, InstanceNorm, and SyncBatchNorm shape inference silently overwrite caller-provided bad gamma/beta shapes, unlike LayerNorm's `SHAPE_ASSIGN_CHECK`. | Use checked shape assignment for provided gamma/beta shapes and add symbolic shape-inference regressions. |
| XOP6 | Open | Hidden-output metadata | InstanceNorm has hidden mean/var outputs consumed by backward but lists only `output`; CTCLoss registers `FListOutputNAmes` with a typo, so its hidden grad output names are not exposed through the standard attr. | Normalize hidden-output metadata for InstanceNorm and fix the CTCLoss output-name attr typo; add `list_outputs()` coverage. |
| XOP7 | Open | oneDNN output req/copyback | Several oneDNN paths directly call `CreateDNNLData`/`GetDNNLData` instead of `CreateDNNLMem` plus `CommitOutput`, so `kNullOp`, `kAddTo`, view outputs, and descriptor-copyback can be wrong. Confirmed/suspicious sites include DNNL BatchNorm forward, LayerNorm mean/std, activation backward, masked softmax, softmax/log-softmax forward, deconvolution weight grad, quantized BatchNorm, and quantized fused subgraph conv. | Add a reusable DNNL output-req/view test harness and convert direct-output sites to req-aware allocation or explicit unsupported-req checks. |
| XOP8 | Open | Quantized range outputs | Multiple native and DNNL quantized ops overwrite scalar min/max or scale/shift outputs without honoring `req`: quantize, quantize_v2, quantize_asym, requantize, quantized reshape, flatten, activation, pooling, convolution, fully connected, BatchNorm, elemwise add/mul, embedding, concat, quantized batch-dot, and oneDNN transformer self-attention subgraphs. Quantized transpose has the better req-aware pattern. Some recently touched oneDNN conv/FC subgraph paths skip `kNullOp` for min/max but still do not implement independent min/max requests or `kAddTo`. | Apply the quantized-transpose range-output helper broadly and add parameterized `kNullOp`/`kAddTo` tests for range outputs, including mixed requests such as `[write,null,null]` and `[write,add,add]`. |
| XOP9 | Open | Stochastic/resource ops | cuDNN/MKL Dropout forward/backward write directly and do not honor the same `req` semantics as the native dropout paths; RNN dropout relies on reserve-space state and should be covered with add/null gradient requests. | Add focused Dropout/RNN dropout req tests for native, cuDNN, MKL where available; either support or explicitly reject unsupported requests before writing. |
| XOP10 | Open | Aux-state timing | `IdentityAttachKLSparseReg` updates moving averages in backward and its forward uses `req[sparsereg::kData]` for the output request. This mirrors the same state-timing and request-index bugs found in normalization operators. | Use the output request in forward; decide whether moving-average updates belong in forward; add forward-only and request-type tests. |
| XOP11 | Open | Gluon affine flags | Gluon `LayerNorm(center=False/scale=False)` still passes stored beta/gamma values; GroupNorm correctly substitutes zeros/ones. | Match GroupNorm behavior and add a disabled-affine regression for LayerNorm. |
...
| XOP13 | Open | General output-request semantics | Additional non-DNNL operators ignore or misuse output/gradient requests: LRN backward writes gradients directly regardless of `req`; BoxNMS forward/backward writes visible/hidden outputs and zeroes input grads directly; image resize drops `req`; random crop applies the final output request to a hidden temp buffer; `_npi_average(returned=True)` returns early on `req[0] == kNullOp` before writing a requested second output; TopK `ret_typ='mask'` zeroes output before checking `kNullOp`; `_npi_unique` checks only `req[0]`, force-initializes optional outputs, and writes index/inverse/count outputs without honoring their individual requests; `sample_unique_zipfian` writes both outputs without request checks; empty-input NumPy reductions fill outputs directly instead of using `req`. | Add focused reproductions for each operator, then either implement full `req` support or reject unsupported requests before touching outputs. Each fix must add a regression that pre-fills outputs with sentinels and verifies `kNullOp` preservation plus `kAddTo` accumulation, including multi-output cases with mixed requests such as `[write,null,add]`. Include these in the reusable XOP12 contract suite where possible. |
| XOP14 | Open | cuDNN/library beta mapping | Several cuDNN wrapper paths pass library `beta=0` and write directly instead of mapping `OpReqType` to library accumulation/null behavior. Confirmed/suspect sites include cuDNN LRN forward/backward, cuDNN activation forward/backward, cuDNN spatial transformer forward/backward, cuDNN bilinear sampler forward and grid gradient, cuDNN pooling forward/backward, and softmax-activation forward/backward; convolution/deconvolution show the safer `req == kAddTo ? beta_add : beta` pattern. | Audit all cuDNN/cuBLAS/cuTensor wrapper beta/output-pointer sites. For each path, either map `kAddTo` to beta=1 and skip `kNullOp`, or assert unsupported requests before writing. Every cuDNN fix must add GPU backend-specific tests for `grad_req='add'`/`'null'` and forward `out=` sentinel preservation where applicable, comparing against native GPU behavior. |
| XOP15 | Open | Quantized primary-output req | Native quantization kernels also ignore `req[0]` for the primary data output, not just range outputs. `_contrib_quantize`, float-input `_contrib_quantize_v2`, and `_contrib_dequantize` launch kernels directly into `outputs[0]`; `_contrib_quantized_elemwise_mul` writes every output directly. This breaks `kNullOp` preservation and `kAddTo` accumulation. | Add direct operator contract tests with prefilled sentinels for data and range outputs, then make primary-output kernels req-aware or reject unsupported requests before writing. The regression set must cover `req[0]=kNullOp`, `req[0]=kAddTo`, and mixed data/range requests for each affected op. |
| XOP16 | Open | Quantized inference contracts | `_contrib_quantized_elemwise_mul` logs a type error for non-int8 lhs/rhs but still returns successful type inference, while forward unconditionally reads both inputs as `int8_t`. Its storage inference also force-assigns every input/output to default storage, masking sparse inputs despite only registering dense CPU compute. `_contrib_quantized_embedding` selects `kFComputeEx` for row-sparse weight but registers only `FCompute<cpu>` and leaves range-output storage incomplete in that branch. | Tighten type inference to fail unsupported dtypes, reject or correctly dispatch sparse storage, and add type/storage inference regressions for uint8/float inputs plus row-sparse/CSR cases. The tests must assert that invalid dtype/storage combinations fail during inference or bind, not later by reinterpretation or missing dispatch. |
| XOP17 | Open | Quantized metadata | Quantized RNN lists `statecell_ouput` instead of `statecell_output` when `state_outputs=True`, adding another output-name drift case in quantized code. | Fix the spelling while preserving compatibility if necessary, and add `list_outputs()` coverage for quantized RNN state outputs that asserts the canonical state-cell name. |
| XOP18 | Open | Quantized subgraph req/backward | oneDNN self-attention QK/ValAtt subgraphs write quantized min/max outputs directly and override BF16 fallback temporary requests with all-`kWriteTo` vectors. The same transformer subgraph registrations use `MakeZeroGradNodes`, extending the full-QAT backward gap beyond FC/Conv to self-attention and batch-dot-style quantized subgraphs. | Cover `_sg_onednn_selfatt_qk{,_split}` and `_sg_onednn_selfatt_valatt` in req tests; include transformer/batch-dot subgraphs in the full QAT backward design tracked by B4. Tests must include quantized range-output sentinels, primary-output `kNullOp`/`kAddTo`, and backward tests that fail while gradients are silently zeroed. |
| XOP19 | Open | oneDNN descriptor/output handling | Additional oneDNN wrappers still bypass full output-request/copyback handling. `DNNLSoftmaxForward` and `DNNLLogSoftmaxForward` reject `kAddTo` but use `GetDNNLData(&dst_desc)` directly and do not `CommitOutput`; `DNNLReduceFwd::Execute` writes scalar outputs through a raw oneDNN memory over `data().dptr_` and non-scalar outputs through `GetDNNLData(&desc)` without request-aware allocation; `DNNLMaskedSoftmax` accepts `req` but writes output memory directly as intermediate scratch; DNNL deconvolution weight-grad fast path returns direct `CreateDNNLData`; DNNL reshape uses raw/default memory paths without copyback; quantized oneDNN FC/conv/transformer subgraphs bind primary output storage directly; quantized oneDNN batch-dot writes min/max directly. BF16 fallback paths in DNNL FC, subgraph conv, and transformer force temporary requests to all `kWriteTo`, losing caller `kNullOp`/`kAddTo` semantics. | Convert or explicitly gate these paths with the same contract tests as XOP7: null/add, views, descriptor changes, scalar outputs, quantized primary/range outputs, and BF16 fallback copyback. Each repaired oneDNN path must add a DNNL-specific regression using view outputs or descriptor-changing outputs plus sentinel checks for `kNullOp` and `kAddTo`. |
