...
| P0 | D2L-Bug-3 | `natural-language-inference-bert.ipynb` dead kernel 1095s into training (no traceback). FATAL message now carries full allocation context (requested bytes, pool used, device free/total, retries attempted) so a future kernel death leaves a usable trail in stderr. Solo BERT NLI run on `.20260523.5` started 06:21 (timeout 1500s) to determine whether the symptom was a same-config OOM under the D2L-Bug-2 path (now retried) or a genuine native fault. | If solo BERT NLI passes: D2L-Bug-3 is resolved by the D2L-Bug-2 retry path. If it fails: needs cuda-gdb + coredump capture (root-cause requires raising `ulimit -c` and either disabling apport or pointing `kernel.core_pattern` at a file). |
