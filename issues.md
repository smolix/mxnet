...
| GH5 | In progress | Operator correctness | Stale operator PRs cover deformable-conv `slice_axis`, `linspace`, fp16 `argsort`, GroupNorm, NumPy inf reductions, and randomized ReLU. CPU `mx.nd.argsort` now accepts float16 inputs through the ordering dispatch path, with focused coverage for ascending/descending axes and int32/int64 index outputs. `mx.nd.linspace(..., dtype='int32')` now matches NumPy integer flooring semantics for negative and descending ranges and preserves exact endpoint values, with focused CPU/GPU-capable coverage. | Continue with the remaining focused CPU/GPU operator items after the CUDA smoke batch. |
