...
| GH4 | In progress | Resource hygiene | Logger/file-handle lifetime and repeated `Extract()` leaks are separate from the engine/DataLoader lifecycle work. `MXIndexedRecordIO.open()` now closes the native RecordIO handle and any opened index file if index loading fails after opening the underlying record file, with focused regression coverage. | Continue leak/file-handle regression tests around the remaining utility paths; also reduce the existing `recordio.py` `label.tostring()` warning flood separately from the leak fix. |
