...
4. **`test_pos_single_concat_pos_neg[int8/auto-data_shape1]`** — re-tested 2026-05-18 against HEAD `b98ddf787`: failure persists with `MXNET_TEST_SEED=11`. Failure mode: max int8 quantization error 5.0 on output (atol=0.12, so 40× exceeded) where most error locations have `a=0, b≈1.2` — entire output channels are being zeroed. data_shape1 is `(4, 3, 24, 24)`; channels 3-6 of output (the `relu(conv0(x))` half of the concat) are corrupted. Suspect: oneDNN v3 reorder uint8→int8 in `src/operator/quantization/dnnl/dnnl_quantized_concat.cc:106-122`. Math checks out: v3 formula is `dst = (src - src_zp) * src_scale / dst_scale + dst_zp`; we want `dst = src * (out_scale/i_scale)`. **Attempts:** (a) original `set_scales_mask(DNNL_ARG_SRC, 0)` with `out_scale/i_scale` — failing as described; (b) tried switching to `set_scales_mask(DNNL_ARG_DST, 0)` with `i_scale/out_scale` (mirror of `dnnl_quantize_asym-inl.h` fix) — same failure pattern, same channels zeroed. So the math direction isn't the issue. **Next**: need `DNNL_VERBOSE=2` trace to see which engine config oneDNN picks for the uint8→int8 reorder, and inspect whether `rescaled_mem` (via `TmpMemMgr`) is being recycled before `DNNLStream::Submit()`. Possibly the `data_md.push_back(mem_desc)` and the subsequent `DNNLConcatFwd::GetCached` is caching a stale desc.
