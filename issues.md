...
| L8 | In progress | Build freshness | Local DNNL quantized conv+sum and compiler-noise edits rebuilt successfully; `build/libmxnet.so` was rebuilt again after the host reboot and submodule cleanup at 2026-05-21 14:47:01 UTC. The library was incrementally rebuilt again after runtime-container, CUDA Proposal/MultiProposal hardening, and the non-tuning `operator_tune.cc` build fix. It was then reconfigured and fully rebuilt after the legacy OpenCV packaging fix; the rebuilt native library reports commit `12cd3a720200a765ac0e859f0781555f64125a7a`, `CUDA=True`, `OPENCV=False`, and a passing GPU smoke. Focused post-rebuild checks for reducer, packaging, optimizer, CTC, pooling, einsum, TF32 deconv, cuDNN frontend-autotune smoke, cuDNN stream/workspace behavior, cuBLASLt serial and threaded stress, GPU reducer paths, CUDA smoke/regression subsets, DNNL serial shards, broad CPU unittest, runtime containers, Proposal/MultiProposal, embedding default coverage, focused C++/gtest filters, D1 wheel artifact audit, imported NumPy sum operator coverage, RReLU CPU/GPU mask behavior, and the C API symbol/CachedOp/Executor filter pass. The full GPU/operator lane is still incomplete. | Finish broad GPU/operator reruns on this rebuilt library; reconfigure before any later release build if more commits land. |
...
| FS3 | Partial | C++ gtest | `OMPBehaviour.after_fork` now checks the child exit status and passes directly. Focused C++ filters pass: engine/lifecycle `13`, DNNL/NDArray `8`, BatchNorm `16`, narrow operator basics `10`, quiet broad filter excluding topology/perf/timing/imperative/DNNL sweeps `57/57`, `CORE_OP_RUNNER.Execute*` `6/6`, and the C API symbol/CachedOp/Executor combined filter `7/7`. Earlier broad C++ attempts were stopped for runtime/noise in `IMPERATIVE.CopyOp`/`IMPERATIVE.ActOp`, not visible assertions. | Schedule the remaining C++ suite in quieter shards, excluding known long topology cases and overly verbose imperative sweeps. |
...
| O11 | In progress | Variant versioning | Source `python/mxnet/libinfo.py` still carries the prior CUDA local version suffix, but the primary and legacy wheel setup paths now honor `MXNET_PACKAGE_VERSION` so release/staging jobs can stamp an explicit package version without editing source. The GitHub release-wheel workflow now passes the resolved tag version through that variable. The macOS wheel had previously been stamped in the staging tree only to avoid changing CUDA packaging metadata on this branch. After reconfigure, the rebuilt native library and audited CUDA wheel payload report commit `12cd3a720200a765ac0e859f0781555f64125a7a`, while the audit artifact's filename/version remains `2.0.0+cu13.bw.20260518.1` because it was built before the workflow update. `auditwheel show` reports the raw artifact as `linux_x86_64` constrained to `manylinux_2_39_x86_64`. | Rebuild artifacts with a source-specific local version before publication. Decide the broader provenance scheme for commit/dirty state/build options/submodule SHAs, and build in a controlled policy environment before claiming manylinux compatibility. |
...
3. Resolve or document C1-C8; D1-D4 are resolved unless fresh repros arrive,
   keep D5-D7 external, and keep D8 informational unless current artifacts
   produce a concrete MXNet runtime repro.
