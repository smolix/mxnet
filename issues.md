...
|---|---|---|---|---|
| P1 | D2L-Issue-3 | **Narrowed (PM session 2)** | `chapter_computer-vision/fine-tuning.ipynb` multi-GPU SEGV — root cause now isolated. Bisection ladder (all scripts in `d2l-issues/repro_*.py`): two sequential trainings with random GPU tensors → **PASS**; with random CPU tensors then `split_and_load` → **PASS**; with 5-epoch + eval-between-epochs structure → **PASS**; with Gluon `DataLoader` over a **synthetic dataset** (no OpenCV) → **PASS**; with `DataLoader` over `gluon.data.vision.ImageFolderDataset` + standard image augmentation transforms (`RandomResizedCrop` / `RandomFlipLeftRight` / `ToTensor` / `Normalize` — uses OpenCV imdecode + per-item `mx.image` ops on CPU NDArrays) → **CRASH** (faulthandler shows ~26 native threads SEGV simultaneously, classic use-after-free pattern). | Bisect what's specifically broken in the OpenCV imdecode → per-item-CPU-NDArray-ops → multi-GPU `split_and_load` pipeline. Likely candidates: (a) `mx.image.imread` returning an NDArray whose CPU buffer is freed before the engine's CPU→GPU copy completes; (b) per-item augmentation ops queue many tiny `mxnet::Engine` operations whose dependency graph races with hybridized GPU graph compilation; (c) Python-thread iteration over `ImageFolderDataset` competing with engine workers for storage manager mutex. Suggested next narrowing: `ImageFolderDataset` with an identity transform (decode without aug ops) to separate (a) from (b). |
