...
in `tests/python/unittest/test_random.py::test_randint_generator`; treat that as
the next concrete broad-sweep blocker rather than as a GPU driver failure.
...
| FS12 | Open | Direct long shard | A direct long shard aborted around 95% completion without a normal pytest summary after a hard bus error in `tests/python/unittest/test_random.py::test_randint_generator`. The process had been visible to the GPUs; this is not evidence of a driver outage. | Reproduce `test_randint_generator` directly, preferably under gdb or with a narrower random-test shard, then decide whether this is a random-generator memory bug, order-sensitive state corruption from a prior test, or a host-load artifact. |
...
1. Reproduce and resolve FS12's `test_randint_generator` bus error before
   claiming the Python CPU/GPU sweeps are clean.
