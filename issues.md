...
| GH6 | In progress | Data/datasets | Dataset/RecordIO issues remain around transform handling after the RecordIO/ImageFolder slice. The C++ `RecordFileDataset` handle now resets its thread-local RecordIO reader when switching between `.rec` files, `ImageFolderDataset` accepts an explicit `classes=` order for split-stable labels while rejecting invalid class lists, and `MultiBoxPrior` now has focused imperative/symbolic expected-value coverage for explicit `steps`, `offsets`, and `clip=True`. | Continue with transform handling and any fresh dataset repros. |
