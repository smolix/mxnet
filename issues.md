...
| L8 | In progress | Build freshness | Local DNNL quantized conv+sum and compiler-noise edits rebuilt successfully; `build/libmxnet.so` was rebuilt again after the host reboot and submodule cleanup at 2026-05-21 14:47:01 UTC. The library was incrementally rebuilt again after runtime-container, CUDA Proposal/MultiProposal hardening, the non-tuning `operator_tune.cc` build fix, and the native autograd attached-intermediate fix. The last Python library sync happened before the later release/docs/CI commits, and the CMake embedded commit hash needs a fresh reconfigure before publication. Focused post-rebuild checks for reducer, packaging, optimizer, CTC, pooling, einsum, TF32 deconv, cuDNN frontend-autotune smoke, cuDNN stream/workspace behavior, cuBLASLt serial and threaded stress, GPU reducer paths, CUDA smoke/regression subsets, DNNL serial shards, broad CPU unittest, runtime containers, Proposal/MultiProposal, embedding default coverage, focused C++/gtest filters, D1 wheel artifact audit, imported NumPy sum operator coverage, RReLU CPU/GPU mask behavior, the C API symbol/CachedOp/Executor filter, and the NumPy-heavy GPU operator shard pass. The active classic GPU operator shard is still running. | After active Python tests finish, reconfigure, rebuild `mxnet`, sync `python/mxnet/libmxnet.so`, and run fresh CPU/GPU smokes before release artifacts. |
...
| FS3 | Partial | C++ gtest | `OMPBehaviour.after_fork` now checks the child exit status and passes directly. Focused C++ filters pass: engine/lifecycle `13`, DNNL/NDArray `8`, BatchNorm `16`, narrow operator basics `10`, quiet broad filter excluding topology/perf/timing/imperative/DNNL sweeps `57/57`, `CORE_OP_RUNNER.Execute*` `6/6`, the C API symbol/CachedOp/Executor combined filter `7/7`, a combined engine/thread-local/OpenMP/runtime-container/C API/C++ executor/DNNL/BatchNorm shard at `44/44`, and the previously noisy imperative shards: `IMPERATIVE.CopyOp` `1/1`, `IMPERATIVE.ActOp` `1/1`, and `IMPERATIVE.CopyBackwardsOp:IMPERATIVE.ActBackwardsOp` `2/2`. | Continue the remaining C++ suite in quieter shards, excluding known long topology/perf/timing cases. |
...
| O5 | Resolved locally | Changelog | `CHANGELOG.md` now records release notes for the CUDA 13/cuDNN 9/Linux wheel line, oneDNN and quantization validation, Apple Silicon/macOS arm64, and packaging caveats. |
| O6 | Resolved locally | README/docs | README and static-site install/build guidance now describe this maintained fork, Linux CUDA dependency expectations, the OpenCV-off CUDA wheel behavior, and the split between pip-provided cuDNN/NCCL packages and the system CUDA toolkit. |
