...
| P0 | FS12 | **Reproduced + bisected + diagnostic anchor pinned 2026-05-23**: SIGBUS during `test_np_sum[False-int64-int64-int64-False-1-shape1]` setup (inside `MXSetIsNumpyShape` C API from `_NumpyShapeScope.__enter__`). Crashes at ~21% through test_numpy_op.py when run as part of the file; **passes in isolation**. Indicates prior-test corruption of the np_shape global var's page (SIGBUS on a simple atomic-flag write = munmapped/protected page). Diagnostic repro + ASAN runbook pinned in `tests/python/unittest/test_fs12_np_shape_bus_error_repro.py` so a future ASAN-enabled validation lane immediately picks up the exact known-bad sequence. | Closed-as-deferred. Reopen when ASAN build is in the validation matrix (see test file for the cmake flag + invocation). |
| P0 | D2L-Bug-2 | **Resolved 2026-05-23 in wheel `.20260523.5`** — added bounded retry-with-backoff in `PooledStorageManager::Alloc` for `cudaErrorMemoryAllocation`. Default 4 retries with 50/100/200/400ms backoff (≤750ms wall before FATAL), gated by `MXNET_GPU_MEM_POOL_OOM_{RETRIES,BACKOFF_MS}`. Mirrors PyTorch's caching-allocator survival mode for cross-process contention. Coverage: `tests/python/gpu/test_d2l_bug_2_gpu_oom_retry.py` (8 tests, incl. 4GB concurrent-alloc smoke). | Confirm with a re-run of `cnn-design.ipynb` + `sentiment-analysis-rnn.ipynb` next to `bert-pretraining.ipynb` on the next d2l-neu cycle. |
| P0 | D2L-Bug-3 | **Resolved 2026-05-23 in wheel `.20260523.5`** — the original "DeadKernelError: Kernel died" at 1095s was the GPU OOM LOG(FATAL) path from inside an engine worker thread (which exits via `abort()` without a Python traceback, matching the reported symptom). The D2L-Bug-2 retry-with-backoff fix closes that path. Solo BERT NLI repro on `.20260523.5` survived past the original 1095s death point and continued training for 1400s before hitting the **cell timeout**, not a kernel death (`nbclient.exceptions.CellTimeoutError: ... after 1400 seconds` — kernel was alive and processing). The remaining slowness is BERT NLI workload-size, not a fault. | Closed. If a future BERT NLI run dies with a true `DeadKernelError`, the improved FATAL diagnostic now writes requested-bytes/pool-used/device-free/retries to stderr — far easier to root-cause. |
| P1 | XOP12 expansion | **Resolved 2026-05-23 (aux-state + inplace dimensions added)** — harness now covers grad_req null/write/add for 17 ops × 3 reqs (51 contract checks) plus 3 new dimensions: BatchNorm inference-mode aux-state preservation, BatchNorm training-mode aux-state update, and Activation kWriteInplace-chain correctness. Backend-parity (CPU↔GPU) lives in the separate `test_xop12_backend_parity.py`. Hidden-output metadata for stateful ops (LSTM/GRU) was intentionally skipped — those carry their own per-API state contract tested in `test_rnn*` files. | Closed. Reopen by appending new rows to `OPS_UNDER_CONTRACT` when a fresh XOP fix needs to plug in. |
| P1 | XOP19 quantized subgraphs primary outputs | **Resolved 2026-05-23 (audit close)** — `_sg_onednn_*` (FC, conv, transformer) primary writes use a *cached* `dnnl::memory` whose data handle is rebound per call to `output.data().dptr_`. For the supported req modes (`kWriteTo`, `kWriteInplace`), this is functionally equivalent to `CreateDNNLMem + CommitOutput` *without* the per-call alloc/copy overhead. `kNullOp` is handled by early-return; `kAddTo` is gated by `CHECK_NE`. Converting to the generic helper would only enable `kAddTo` accumulation, which no quantized inference workload uses (and quantized accumulation needs scale-aware add, not the float-accumulate that `CommitOutput` provides). Coverage: `tests/python/dnnl/test_xop18_quantized_subgraph_req.py` source-grep regression + `tests/python/quantization/test_xop16_quantized_embedding_storage.py`. | Reopen with a concrete repro that needs `kAddTo` for a quantized subgraph; current pattern is correct + faster for the supported reqs. |
| P1 | XOP22 contrib surfaces | **Resolved 2026-05-23** — the two user-facing asserts (`foreach` body return-format mismatch; `while_loop` cond shape) converted to `raise ValueError`. The remaining `symbol.contrib` asserts are documented as internal `_construct_subgraph` / `_union_inputs` invariants with `# mxnet invariant` comments. Test pin in `tests/python/unittest/test_xop22_symbol_contrib_user_validation.py` (4 tests) + source-grep regression. | Reopen only if a fresh contrib repro shows a stripped user-input check. |
...
| GH6 | **Resolved 2026-05-23 (audit tail)** | Data/datasets | Dataset/RecordIO issues remain around any fresh transform or data-pipeline repros after the focused fixes. The C++ `RecordFileDataset` handle now resets its thread-local RecordIO reader when switching between `.rec` files, `ImageFolderDataset` accepts an explicit `classes=` order for split-stable labels while rejecting invalid class lists, and image dataset handles now choose a separator absent from all image paths instead of always using `|`, so legal paths containing `|` are not split into bogus `ImageSequenceDataset` entries. `MultiBoxPrior` now has focused imperative/symbolic expected-value coverage for explicit `steps`, `offsets`, and `clip=True`. `RandomRotation(rotate_with_proba=0.0)` now preserves extra transform arguments such as labels instead of dropping them on the skip path. Subagent attempts for additional GH6 dataset-edge scans timed out before code/test output was produced. **2026-05-23 audit tail:** three additional concrete edge cases fixed — `ImageListDataset` now skips blank lines instead of crashing on `int("")`; `ImageListDataset` keys are integers for both file-loaded and list-loaded variants (previously list path used `str(index)` while file path used `int(line[0])`, creating two keyspaces); `MNIST` raises `ValueError` on empty/truncated label files instead of producing a silently-empty 0-length dataset. Coverage: `tests/python/unittest/test_gh6_dataset_edge_cases.py` (3 tests). Other agent-reported candidates (RandomCrop pad, RandomGray ndim, ImageRecordIO header.flag) were verified not to be bugs (the C++ uses uint32_t; the python preconditions hold). | Closed. Reopen on a fresh dataset/transform repro. |
