...
long GPU shard is still running as of the latest 2026-05-22 update and should
be recorded here only after it exits with a final pytest summary.
...
| FS8 | In progress | Stale skip audit | Temporary/flaky skips are being removed only when the underlying test can be made meaningful. The profiler stale-skip batch now passes `4 passed` after relaxing custom-op profiler assertions to check actual operator-family entries rather than stale exact names. GPU stale skips for NCCL push/pull, KVStore row-sparse push/pull, and Gluon GPU memory profiler now pass `3 passed`; NCCL now skips only when no GPU or no NCCL feature is present. The old Gluon issue-11164 dynamic reshape/slice tests are mostly active again: the non-deconv repaired subset passes `10 passed`, and the deconv subset passes `6 passed`. | Keep stale skips under suspicion. Do not restore broad skips; either repair the test, add a precise capability guard, or open a concrete runtime bug row. Commit the repaired skip removals after the remaining crashing node is accounted for. |
| FS9 | Open | Gluon BatchNorm crash | Re-enabling `tests/python/unittest/test_gluon.py::test_batchnorm_16c` exposes a hard interpreter crash during the forward comparison path around `check_layer_forward_withinput` / `out1.asnumpy()`. This is not resolved by the stale-skip cleanup and should not be hidden by re-adding the old blanket issue-11164 skip. | Use gdb/native stack narrowing to identify whether the crash is in BatchNorm shape/type handling, NumPy semantics, or cached HybridBlock execution; fix the runtime or test-source bug before counting `test_gluon.py` clean. |
| FS10 | Open | Higher-order gradients | `tests/python/unittest/test_higher_order_grad.py` remains an active blocker. A broad xfail was reverted after review because the file must continue to exercise higher-order gradients. The current isolated failure pattern is many nonzero unary second-order gradients failing shape/copy checks, while some zero-second-derivative unary cases pass; dense expected-shape issues may be separate test-source bugs. | Fix runtime and/or precise test expectations so real higher-order gradients remain exercised. Do not blanket-disable the file; keep any xfail, if ultimately needed, narrow and tied to a concrete unsupported op/path. |
| FS11 | Open | Quantization GPU wrapper | The prior CUDA 304 quantization failures do not reproduce as a driver/runtime failure in direct GPU probes. The GPU wrapper has stale import semantics: it inserts `../quantization`, imports through `from test_quantization import *`, misses the CPU file's private autouse legacy-semantics fixture, and sets the default GPU device after importing tests. Focused direct GPU quantization from the right directory passed simple quantize/conv nodes, while full wrapper coverage still includes unsupported GPU bfloat16 scalar copy, quantized transpose/reshape, CPU-only quantize-model paths, and RNN quantization that explicitly rejects GPU. | Repair the wrapper import/default-device/fixture behavior first, then add precise GPU capability skips or xfails only for truly unsupported quantized ops. |
| FS12 | In progress | Direct GPU shard | A direct pytest shard covering AMP, batchnorm running stats, cuBLASLt strided, device, fusion, KVStore GPU, NCCL, operator GPU, and RTC is still running under tmux and has not produced a final summary yet. The process is visible to all four GPUs and should not be treated as a driver problem. | Poll the shard until it exits; record exact pass/fail/skip counts here and use any failures as the next focused GPU queue. |
...
1. Finish FS12 and record the exact GPU shard summary.
2. Resolve FS9 and FS10 before claiming the Python CPU/GPU sweeps are clean.
3. Repair FS11 so quantization GPU coverage distinguishes wrapper bugs from
   truly unsupported GPU quantized operators.
4. Consume current D2L notebook-run/output-audit artifacts now that the
   standalone GPU and transformer runtime probes are clean; reopen MXNet work
   only for fresh runtime repros.
5. Continue CN2/CN9, C4, B4, T11, and release/packaging rows using focused
   tests before full sweeps.

Before shipping another public Linux/CUDA preview wheel:

1. Re-run the current active full-sweep blockers after FS9-FS11 are repaired.
