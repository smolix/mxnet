...
| XOP19 | Open | oneDNN descriptor/output handling | Additional oneDNN wrappers still bypass full output-request/copyback handling. `DNNLLogSoftmaxForward` rejects `kAddTo` but uses `GetDNNLData(&dst_desc)` directly and does not `CommitOutput`; `DNNLReduceFwd::Execute` writes scalar outputs through a raw oneDNN memory over `data().dptr_` and non-scalar outputs through `GetDNNLData(&desc)` without request-aware allocation; `DNNLMaskedSoftmax` uses output memory as intermediate scratch and rewrites it in-place across multiple primitives. Quantized oneDNN batch-dot also writes min/max directly. | Convert or explicitly gate these paths with the same contract tests as XOP7: null/add, views, descriptor changes, and scalar outputs. |
