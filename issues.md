...
| GH5 | In progress | Operator correctness | Stale operator PRs cover deformable-conv `slice_axis` and GroupNorm after the first focused fixes. CPU `mx.nd.argsort` now accepts float16 inputs through the ordering dispatch path, with focused coverage for ascending/descending axes and int32/int64 index outputs. `mx.nd.linspace(..., dtype='int32')` now matches NumPy integer flooring semantics for negative and descending ranges and preserves exact endpoint values, with focused CPU/GPU-capable coverage. RReLU training forward now samples into the mask buffer without shadowing the output tensor and requests the needed random/temp resources; focused CPU regression and a direct GPU smoke verify the output uses the sampled mask. NumPy min/max reductions now have focused CPU coverage for all-`+inf`, all-`-inf`, and mixed-infinity inputs across float16/float32/float64. | Continue with the remaining focused CPU/GPU operator items after the CUDA smoke batch. |
