...
checked-arithmetic paths pass. The broad CPU/DNNL/GPU reruns
...
| L5 | Resolved | Tracker cleanup | Duplicate issue trackers and stale markdown reports have been imported here or removed from the repo. Current D2L diagnostic logs are treated as local artifacts; only executable repro tools should be retained intentionally. | Keep `issues.md` as the processing queue; retain only active investigation notes and executable d2l repro tools. |
