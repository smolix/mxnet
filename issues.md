...
single-machine kvstore fix + release-wheel timeout/thread update)
**Latest tag on GitHub:** `v2.0.0+cu13.bw.20260525.3` (release published with
CUDA wheel)
**Release wheel:** <https://github.com/smolix/mxnet/releases/download/v2.0.0%2Bcu13.bw.20260525.3/mxnet-2.0.0%2Bcu13.bw.20260525.3-cp312-cp312-linux_x86_64.whl>
**Local wheel:** `dist/mxnet-2.0.0+cu13.bw.20260526.3-cp312-cp312-linux_x86_64.whl`
(522 MiB, smoke-tested locally; `rt.get_commit_hash()` now reports
`cfe0fbd6881be1834e96dcd638aa95de5dc8856d`. Not a release artifact yet
because the tracked working tree is dirty.)
...
|---|---|---|---|---|
| P0 | V1 | Open validation gap | Full Python unit-test and d2l-neu notebook sweeps have **not** been re-run end-to-end on the latest release wheel `v2.0.0+cu13.bw.20260525.3`. Targeted release verification is green: clean-wheel import/GPU conv smoke, no NumPy subnormal warnings, `test_cuda_runtime.py`, D2L AlexNet/NiN/VGG after the cuDNN fix, and focused regression coverage for the leak/image-pipeline/dropout fixes. Local wheel `2.0.0+cu13.bw.20260526.3` imports cleanly with warnings promoted to errors, reports the current HEAD commit, runs the mixed int/float16 floor-divide boundary correctly, and runs GPU sum + cuDNN convolution on GPU 2. It remains smoke-only until rebuilt from a clean tracked tree. Older `wheel-test-20260522T215546Z` logs predate the current fixes and include missing test deps (`scipy`) plus old oneDNN 0-dim failures, so they are not current failure evidence. | Re-run the full sharded unit-test matrix from the released `.3` or next clean wheel with complete test deps, then re-run the full d2l-neu MXNet notebook sweep pinned to that wheel. |
...
| P2 | O1, O4, O7 | Partial / Open | Linux wheel now has a working NVIDIA-pip runtime layout and published release artifact, but it is still not a self-contained CUDA/cuDNN/NCCL wheel (O1). Release publication was manual; GitHub release automation is still absent (O4). No conda/system package story exists (O7). Local working-tree guard now refreshes/rebuilds `build/` before cleanup-wheel packaging and `release_provenance.py` rejects CMake metadata whose `MXNET_COMMIT_HASH` differs from `git HEAD`; this needs commit/merge before it counts as release process hardening. | Strategic; decide whether to keep the pip-runtime dependency model or produce a bundled runtime wheel; automate release upload and commit the build-metadata guard if this branch becomes recurring release work. |
