...
4. **`test_pos_single_concat_pos_neg[int8/auto-data_shape1]`** — **ROOT CAUSE LOCALISED 2026-05-18, oneDNN upstream bug.** The channel-zero symptom is NOT in the qconcat reorder — it's in oneDNN's **`jit_uni_int8_1x1:avx2` 1x1 conv kernel** when fused with `eltwise_relu` post-op AND `ic < simd_w` (AVX2 simd_w=8). The failing shape has `ic=3`; passing shapes have ic=4 (mb=64) and ic=16 (mb=1). The qconcat downstream faithfully copies already-wrong values out of the conv. Both `set_scales_mask(DNNL_ARG_SRC,0)`, `(DNNL_ARG_DST,0)`, and a u8→f32→s8 intermediate decomposition were tried in `src/operator/quantization/dnnl/dnnl_quantized_concat.cc` — all produced identical channel-zero output, confirming the reorder is innocent. **Workaround**: `MXNET_DISABLE_ONEDNN_FUSE_CONV_RELU=1` (passes 6/6 for all data_shapes + seeds). **Upstream**: bug lives in `3rdparty/onednn/src/cpu/x64/jit_uni_x8s8s32x_1x1_conv_kernel.cpp` — `jcp.ic` is padded to `simd_w` via `rnd_up(ic, simd_w)` but the tail handling around `load_bytes(..., ic_tail_size=3)` mis-orders with the s8s8 compensation (`vmm_shift`) when an eltwise_relu injector is appended. **Proposed in-tree fix**: gate `jit_uni_int8_1x1:avx2` at the subgraph-quantize level in `src/operator/subgraph/dnnl/dnnl_conv_property.h` to reject conv+relu fusion on AVX2 hosts when `ic < 8`. Full investigation in `.investigations/qconcat_fix_v2.patch`. Same root cause likely explains #49 (`test_self_attention[*]` 32/32 + `test_batch_dot[*]` 16/16, all int8 matmul subgraph) and #2 regressed (`test_quantize_gluon_with_forward` segfault under DNNL).
