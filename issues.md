...
| L8 | In progress | Build freshness | Local DNNL quantized conv+sum and compiler-noise edits rebuilt successfully; `build/libmxnet.so` was rebuilt again after the host reboot and submodule cleanup at 2026-05-21 14:47:01 UTC. Focused post-rebuild checks for reducer, packaging, optimizer, CTC, pooling, einsum, TF32 deconv, GPU reducer paths, CUDA smoke/regression subsets, DNNL serial shards, broad CPU unittest, and focused C++/gtest filters pass. The full GPU/operator lane is still incomplete, and CMake still embeds the older `MXNET_COMMIT_HASH` value from `b881a9c5a` because the build was not reconfigured. | Finish broad GPU reruns on this rebuilt library; reconfigure before packaging, publishing artifacts, or answering binary provenance questions so embedded metadata and wheel tags match the release commit. |
...
| C3 | Partial | cuBLASLt | Direct cuBLASLt GEMM, FC, dtype, and strided tests pass together: `27 passed, 7 skipped`. A GPU1 environment-prefixed rerun failed with CUDA 304 in subprocess children, matching the known restricted command-context artifact and not a runtime regression. The shared-workspace lifetime/race risk still needs real stress coverage. | Run the cuBLASLt subset under multi-threaded and multi-GPU load using a command shape that preserves child GPU access; keep CUDA 304 environment-prefix failures out of correctness accounting. |
...
| C5 | Partial | cuDNN frontend | Representative cuDNN convolution/deconvolution smoke now passes on Ada, and the higher-risk large-channel probes `test_convolution_large_c` and `test_deconvolution_large_c` also pass. Explicit frontend-autotune fallback/no-plan behavior is still not forced or validated. | Force frontend autotune on representative shapes and verify no-plan cases degrade cleanly rather than aborting. |
