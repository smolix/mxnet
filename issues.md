...
| P1 | XOP19 quantized subgraphs + BF16 fallback | `_sg_onednn_*` (FC, conv, transformer) bind primary output storage directly; BF16 fallback in DNNL FC / subgraph conv / transformer forces caller req to all-`kWriteTo`, losing kNullOp/kAddTo. | Convert to `CreateDNNLMem` + `CommitOutput` with explicit req handling; plug each path into XOP12 harness. |
...
| XOP19 | Partial | oneDNN descriptor/output handling | Reducer, softmax/log-softmax, batch-dot, deconv weight-grad fast path, dnnl_reshape entry (gates `kNullOp`/`kAddTo` at `DNNLReshapeForward`), and `DNNLMaskedSoftmax` entry (kNullOp early-return + kAddTo CHECK_NE, contract pinned in `test_xop19_masked_softmax_req.py`) are converted or properly gated.  Remaining: quantized oneDNN FC/conv/transformer subgraphs bind primary output storage directly; BF16 fallback paths in DNNL FC, subgraph conv, and transformer force temporary requests to all `kWriteTo`, losing caller `kNullOp`/`kAddTo` semantics. | Convert the remaining quantized subgraph + BF16 fallback paths.  Plug each into XOP12. |
| XOP21 | Partial | Large-tensor size truncation | LayerNorm already uses `index_t`; GroupNorm `N`-counter, ROIAlign forward/backward and PSROIPool forward/backward `count`-launch counters are now INT_MAX-guarded.  Remaining: bilinear/spatial transformer paths, image-random kernels, NumPy indexing/linalg, FC/dot helpers, transformer helpers. | Continue the cast audit in the listed remaining files. |
