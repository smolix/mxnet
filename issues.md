...
| P0 | FS12 | **Reproduced 2026-05-23**: hard SIGBUS during `test_numpy_op.py` setup around 26% through the file, inside `MXSetIsNumpyShape` C API (called from `_NumpyShapeScope.__enter__` → `_with_np_shape` decorator).  The crashing call is *test setup*, not `test_randint_generator` proper — the original report misattributed the location.  Indicates prior-test memory corruption of the np_shape global state.  Apport intercepts cores so no backtrace yet. | Continue bisecting test_numpy_op.py to identify the predecessor that corrupts state.  Once isolated, run under valgrind or rebuild with ASAN. |
