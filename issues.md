...
| P2 | O4, O7 | Partial / Open | Linux wheel publication is still manual; GitHub release automation is absent (O4). No conda/system package story exists (O7). Python wheels intentionally rely on NVIDIA pip runtime packages, with import-time preloading and RUNPATH guards now covering CUDA/cuDNN/NCCL resolution. | Defer expensive CUDA release automation until the release cadence is clearer; separately decide whether to keep the pip-runtime dependency model or produce a directly bundled runtime wheel. |
...
- Push `fix-bf16-dnnl-unittest-bugs`, create the GitHub PR, tag the tested wheel, and upload `dist/mxnet-2.0.0+cu13.bw.20260529-cp312-cp312-linux_x86_64.whl`.
- Keep release provenance aligned with the final pushed commit after this tracker update.
- Leave low-priority shell execution in examples/benchmarks for a later pass.
- Follow up separately on warning-only compile clusters in `index_copy.cu`, `sparse_retain.cu`, dmlc concurrent queue sentinel conversion, and oneDNN ITT executable-stack metadata.
...
|---|---|---|
| 2026-05-29 | **CPU FP16 convolution/deconvolution accumulation** | Closed locally in commit `822fcf946`. CPU FP16 convolution now stages im2col/col2im and GEMM accumulation through float workspaces, preserves `kAddTo`/`kNullOp` semantics, and avoids half-precision accumulation NaNs in backward data/weight/bias paths. Deconvolution has a CPU half backward specialization with float bias summation, and the 1x1 generic convolution backward-data path now honors `data_grad_req`. Regression coverage: `test_cpu_fp16_convolution`, grouped/dilated+bias FP16 convolution, deconvolution forward/backward-bias FP16 checks, and a source-invariant test for float workspace usage. Focused source and wheel FP16 tests passed; full wheel acceptance passed. |
| 2026-05-29 | **BF16 / oneDNN / QAT coverage on current wheel** | Closed for the `2.0.0+cu13.bw.20260529` wheel built from `822fcf946`. The wheel acceptance run passed DNNL AMP/BF16/operator/subgraph/QAT shards: `dnnl_amp` `79 passed`, `dnnl_bf16_operator` `22 passed`, `dnnl_subgraphs_matmul` `64 passed`, `dnnl_subgraphs_conv` `429 passed`, and `dnnl_subgraphs_qat_backward` `21 passed`. The CPU runtime feature report includes F16C and oneDNN enabled. |
| 2026-05-29 | **oneDNN uint8 source-zero-point subgraphs** | Closed locally in commit `6d4568d23`. Quantized oneDNN Conv/FC subgraphs no longer reject uint8 inputs with nonzero `data_min`; they compute and bind source zero points through the subgraph primitive path. Regression coverage checks both source invariants and runtime DNNL zero-point binding; full wheel `cpu_unittest`, DNNL quantization, DNNL subgraph conv, and quantization-general shards passed. |
| 2026-05-29 | **CUDA/cuDNN/NCCL runtime library resolution** | Closed locally in commit `3ebafae2c` plus the release bundling step. `mxnet._cuda_runtime` preloads package-local CUDA runtime libraries before `libmxnet.so` loads, and `bundle_runtime_libs.py` stamps `libmxnet.so` with RUNPATH entries for `$ORIGIN/lib`, `scipy_openblas32`, `nvidia/cudnn/lib`, `nvidia/nccl/lib`, `nvidia/cu13/lib`, and local CUDA fallback directories. Release provenance validates `libcudnn.so.9` and `libnccl.so.2` dependencies plus cuDNN/NCCL RUNPATH coverage. |
| 2026-05-29 | **Wheel `2.0.0+cu13.bw.20260529` acceptance** | Closed locally for wheel `dist/mxnet-2.0.0+cu13.bw.20260529-cp312-cp312-linux_x86_64.whl`, built from commit `822fcf946`. Provenance reports tracked tree clean, CUDA/cuDNN/NCCL/oneDNN enabled, OpenCV disabled, matching CMake metadata, and runtime RUNPATH coverage. Full wheel acceptance passed: CPU broad unittest `12557 passed, 788 skipped, 1 xfailed`; CPU `test_operator.py` `1104 passed, 6 skipped`; DNNL/quantization/BF16/QAT shards all passed; GPU NumPy operator shard `10354 passed, 711 skipped`; GPU classic operator shard `1939 passed, 12 skipped`; GPU quantization wrapper `40 passed, 13 xfailed`. Report directory: `wheel-test-20260529T041517Z`. |
