...
| XOP8 | Open | Quantized range outputs | Multiple DNNL quantized ops overwrite scalar min/max outputs without honoring `req`: quantized reshape, flatten, activation, BatchNorm, and related range-output paths. Quantized transpose has the better req-aware pattern. | Apply the quantized-transpose range-output helper broadly and add parameterized `kNullOp`/`kAddTo` tests for range outputs. |
