...
| L3 | In progress | CUDA regression batch | Targeted CUDA tests have cleared cuDNN/TF32 deconv, cuDNN large-channel conv/deconv probes, cuDNN frontend-autotune smoke, cuDNN stream/workspace regression coverage, cuBLASLt GEMM/FC/dtype/strided checks plus a same-process 4-GPU threaded cuBLASLt stress, fp16 batch-dot parity, linalg temp storage, deferred-compute GPU, reducer regressions, NumPy einsum GPU, KVStore local/device/GPU checks, NCCL multiprocess, histogram GPU edge cases, Proposal/MultiProposal checked-arithmetic coverage, a 26-node bounded GPU operator micro-shard, and the NCCL single-process bandwidth check after converting the hard bandwidth gate into a metric. Extension GPU tests now skip when optional shared libraries were not built. | Shard the full GPU lane and run the remaining focused CUDA backlog under C4-C5. Use direct pytest/Python commands for GPU access in this tooling context. |
...
| C5 | Partial | cuDNN frontend | Representative cuDNN convolution/deconvolution smoke now passes on Ada, and the higher-risk large-channel probes `test_convolution_large_c` and `test_deconvolution_large_c` also pass. Forcing `MXNET_CUDNN_AUTOTUNE_FRONTEND=1` inside a direct pytest process validates frontend plan selection on a large-C NCW convolution, including a zero-workspace variant; on this Ada/cuDNN stack those shapes still return usable plans rather than exercising the fallback path. Explicit no-plan fallback behavior is still not forced or validated. | Keep frontend-autotune smoke in CUDA coverage, then either find a real Ada no-plan shape or add a narrow test hook before closing fallback/no-plan behavior. |
