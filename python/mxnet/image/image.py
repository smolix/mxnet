...
def imrotate(src, rotation_degrees, zoom_in=False, zoom_out=False):
    ...
    rotation_degrees = (rotation_degrees.to_device(src.device)
                        if hasattr(rotation_degrees, 'to_device')
                        else rotation_degrees.as_in_context(src.device))
    ...
    wscale = (float(w - 1) / 2)
    h_matrix = (
        nd.repeat(nd.arange(h, ctx=src.device).astype('float32').reshape(h, 1), w, axis=1) - hscale
    ).expand_dims(axis=0)
    w_matrix = (
        nd.repeat(nd.arange(w, ctx=src.device).astype('float32').reshape(1, w), h, axis=0) - wscale
    ).expand_dims(axis=0)
    ...
    h, w = nd.array([h], ctx=src.device), nd.array([w], ctx=src.device)
