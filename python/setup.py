...
#
# Note: the build toolkit's cuDNN minor need not exactly match the minor this
# pin resolves to at install time. cuDNN 9.x is minor-version ABI-compatible in
# both directions, and the runtime check (src/base.cc) only warns on a *major*
# mismatch, so a wheel compiled against 9.23 running against a pip-resolved 9.22
# (or vice-versa) is silent and correct (OI-20, resolved). Only a cuDNN 10.x
# jump would re-trigger the warning and warrant bumping the `<10` ceiling.
...
# Stamp the resolved package version into a generated `_build_info.py` so
# `mxnet.__version__` at runtime matches the wheel's METADATA `Version:`
# field.  Without this the in-Python value lags the wheel name whenever
# MXNET_PACKAGE_VERSION is set without simultaneously bumping the literal
# in `mxnet/libinfo.py`, which is how the d2l team kept seeing
# `2.0.0+cu13.bw.20260518.1` on freshly-built `.20260522.*` wheels.
_build_info_py = os.path.join(CURRENT_DIR, 'mxnet', '_build_info.py')
with open(_build_info_py, 'w') as _f:
    _f.write('# Generated by setup.py — do not edit, do not commit.\n')
    _f.write('__version__ = {!r}\n'.format(__version__))
