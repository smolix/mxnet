...
class P3StoreDist : public KVStoreDist {
 public:
  explicit P3StoreDist(bool use_device_comm) : KVStoreDist(use_device_comm) {
  ...
  void SetGradientCompression(
      const std::vector<std::pair<std::string, std::string>>& kwargs) final {
 ...
 private:
  ...
  void PullRowSparse_(const int key,
