...
namespace ndarray {
template <>
void Copy<cpu, gpu>(const TBlob& from,
                    TBlob* to,
                    Context from_ctx,
                    Context to_ctx,
                    RunContext ctx) {
...
template <>
void Copy<gpu, cpu>(const TBlob& from,
                    TBlob* to,
                    Context from_ctx,
                    Context to_ctx,
                    RunContext ctx) {
...
template <>
void Copy<gpu, gpu>(const TBlob& from,
                    TBlob* to,
                    Context from_ctx,
                    Context to_ctx,
                    RunContext ctx) {
  if (from_ctx.dev_id == to_ctx.dev_id) {
    ...
    MSHADOW_TYPE_SWITCH_WITH_BOOL(to->type_flag_, DType, {
      if (to->type_flag_ == from.type_flag_) {
        mshadow::Copy(to->FlatTo1D<gpu, DType>(s), from.FlatTo1D<gpu, DType>(s), s);
      } else {
    ...
    CUDA_CALL(cudaMemcpyPeerAsync(to->dptr_,
                                  to_ctx.dev_id,
                                  from.dptr_,
                                  from_ctx.dev_id,
  ...
  MSHADOW_TYPE_SWITCH(out->dtype(), DType, {               // data type
    MSHADOW_IDX_TYPE_SWITCH(out->aux_type(kIdx), IType, {  // row_idx type
      ...
      cub::DeviceScan::InclusiveSum(
          d_temp_storage, temp_storage_bytes, row_flg, row_flg, num_rows, stream);
      mshadow::Tensor<gpu, 1, char> workspace = rsc.get_space_typed<gpu, 1, char>(
          mshadow::Shape1(num_rows * sizeof(IType) + temp_storage_bytes), s);
      ...
      for (const auto& nd : nds) {
        if (nd.storage_initialized()) {
          ...
          num_threads             = nd_nnr;
          mxnet_op::Kernel<MarkRspRowFlgKernel, gpu>::Launch(
              s, num_threads, row_flg, nd_row_idx, nd_nnr);
        }
      }
      ...
      CUDA_CALL(cudaMemcpyAsync(
          &nnr_out, &row_flg[num_rows - 1], sizeof(dim_t), cudaMemcpyDeviceToHost, stream));
      ...
      mxnet_op::Kernel<FillRspRowIdxKernel, gpu>::Launch(
          s, num_threads, out_row_idx, row_flg, num_rows);
      ...
      for (const auto& nd : nds) {
        if (nd.storage_initialized()) {
          ...
          mxnet_op::Kernel<ElementWiseRspAdditionKernel, gpu>::Launch(
              s, num_threads, out_data, row_flg, nd_row_idx, nd_data, nd_nnr, row_length);
        }
      }
    });
  });
    ...
    for (size_t i = 0; i < nds.size(); ++i) {
      ...
      switch (nd.storage_type()) {
        case kDefaultStorage: {
          Kernel<op_with_req<mshadow_op::plus, kWriteTo>, gpu>::Launch(s,
                                                                       out_data.Size(),
                                                                       out_data.dptr<DType>(),
                                                                       out_data.dptr<DType>(),
                                                                       nd_data.dptr<DType>());
...
template <>
void Eval<gpu>(mshadow::Stream<gpu>* s, const real_t val, const NDArray& dst) {
  NDArray temp                   = dst;
