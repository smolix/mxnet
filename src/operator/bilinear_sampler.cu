...
template <typename DType>
__global__ void BilinearSamplerForwardKernel(const int i_c,
                                             const int i_h,
                                             const int i_w,
                                             const DType* data,
                                             const DType* grid,
...
__global__ void BilinearSamplerBackwardKernel(const int i_c,
                                              const int i_h,
                                              const int i_w,
                                              const DType* grad,
                                              const DType* data,
...
inline void BilinearSamplerForward(const Tensor<gpu, 4, DType>& output,
  ...
  // XOP21: guard the int-typed kernel launch block count against silent
  // truncation when the output exceeds INT_MAX elements.
  ...
  CHECK_LE(max_block_full, static_cast<index_t>(std::numeric_limits<int>::max()))
      << "BilinearSamplerForward output element count " << out_size
...
#else
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, { op = new BilinearSamplerOp<gpu, DType>(param); })
#endif  // MXNET_USE_CUDNN
  return op;
}
