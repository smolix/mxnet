...
struct MPAdamWKernel {
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    float var                 = var_data[i] =
        param_beta2 * var_data[i] + (1.0f - param_beta2) * mshadow_op::square::Map(scaled_grad);

    w -= param_eta *
         (param_lr * mean / (mshadow_op::square_root::Map(var) + param_epsilon) + param_wd * w);
    weight32[i] = w;
  ...
  static inline void Forward(const nnvm::NodeAttrs& attrs,
    ...
    MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
      ...
      MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {
        Kernel<MPAdamWKernel<req_type>, xpu>::Launch(s,
                                                     weight.shape_.Size(),
                                                     out.dptr_,
                                                     mean.dptr_,
                                                     var.dptr_,
...
struct MultiMPAdamWKernel {
  template <typename DType>
  MSHADOW_XINLINE static void Map(int i,
                                  const MultiAdamKernelParam<DType, MPDType>& param,
                                  const OpReqType req,
                                  const float rescale_grad) {
...
static inline void MultiAdamWUpdate(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    ...
    Kernel<MultiMPAdamWKernel<MPDType, !std::is_same<DType, MPDType>::value>, xpu>::Launch(
...
inline void multiMPUpdate(const nnvm::NodeAttrs& attrs,
  ...
  if (!MP)
    MultiAdamWUpdate<xpu, Adam_type_identity, 4>(attrs, ctx, inputs_wo_scale, req, outputs, scalef);
  else
