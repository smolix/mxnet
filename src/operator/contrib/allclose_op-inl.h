...
struct AllCloseParam : public dmlc::Parameter<AllCloseParam> {
  ...
  DMLC_DECLARE_PARAMETER(AllCloseParam) {
    DMLC_DECLARE_FIELD(rtol).set_default(1e-05).describe("Relative tolerance.");
    DMLC_DECLARE_FIELD(atol).set_default(1e-08).describe("Absolute tolerance.");
    DMLC_DECLARE_FIELD(equal_nan).set_default(true).describe(
        "Whether to compare NaN's as equal. If True, NaN's in A will be considered equal "
...
template <typename xpu>
void GetResultLogical(mshadow::Stream<xpu>* s,
...
void AllClose(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_TYPE_SWITCH(in0.type_flag_, DType, {
    MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {
      Kernel<allclose_forward<req_type>, xpu>::Launch(s,
                                                      num_items,
                                                      workspaceMem,
                                                      in0.dptr<DType>(),
                                                      in1.dptr<DType>(),
