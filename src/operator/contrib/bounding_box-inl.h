...
inline bool BoxNMSShape(const nnvm::NodeAttrs& attrs,
  ...
  if (mxnet::op::shape_is_none(in_attrs->at(0)) && mxnet::op::shape_is_none(out_attrs->at(0))) {
    return false;
  }

  mxnet::TShape& ishape = (*in_attrs)[0];
  int indim             = ishape.ndim();
  CHECK(indim >= 2) << "input must have dim >= 2"
                    << " the last two dimensions are num_box and box_width " << ishape
                    << " provided";
  ...
  CHECK_GE(width_elem, expected) << "the last dimension must have at least 5 elements"
                                 << " namely (score, coordinates x 4) " << width_elem
                                 << " provided, " << expected << " expected.";
  // check indices
  ...
  CHECK(score_index >= 0 && score_index < width_elem)
      << "score_index: " << score_index << " out of range: (0, " << width_elem << ")";
  CHECK(score_index < coord_start || score_index > coord_end)
      << "score_index: " << score_index << " conflict with coordinates: (" << coord_start << ", "
      << coord_end << ").";
  CHECK(coord_start >= 0 && coord_end < width_elem)
      << "coordinates: (" << coord_start << ", " << coord_end << ") out of range:: (0, "
      << width_elem << ")";
  if (param.id_index >= 0) {
    int id_index = param.id_index;
    CHECK(id_index >= 0 && id_index < width_elem)
        << "id_index: " << id_index << " out of range: (0, " << width_elem << ")";
    CHECK(id_index < coord_start || id_index > coord_end)
        << "id_index: " << id_index << " conflict with coordinates: (" << coord_start << ", "
        << coord_end << ").";
...
struct compute_area {
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    if (pos >= static_cast<int>(batch_start[b + 1]))
      return;
...
void NMSApply(mshadow::Stream<cpu>* s,
  ...
  for (int ref = 0; ref < topk; ++ref) {
    ...
    if (num_worker < 1)
      continue;
    Kernel<nms_impl, cpu>::Launch(s,
                                  num_batch * num_worker,
                                  sorted_index->dptr_,
                                  batch_start->dptr_,
                                  buffer->dptr_,
                                  areas->dptr_,
...
struct nms_assign {
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    for (int j = 0; j < k; ++j) {
      ...
      if (pos >= static_cast<int>(batch_start[i + 1]))
        return;
...
void BoxNMSForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    Tensor<xpu, 3, DType> data = inputs[box_nms_enum::kData].get_with_shape<xpu, 3, DType>(
        Shape3(num_batch, num_elem, width_elem), s);
    Tensor<xpu, 3, DType> out = outputs[box_nms_enum::kOut].get_with_shape<xpu, 3, DType>(
        Shape3(num_batch, num_elem, width_elem), s);
    Tensor<xpu, 3, DType> record = outputs[box_nms_enum::kTemp].get_with_shape<xpu, 3, DType>(
    ...
    Tensor<xpu, 1, int32_t> all_sorted_index(
        sorted_index.dptr_ + sorted_index.MSize(), sort_index_shape, s);
    ...
    areas = 0;
    Kernel<compute_area, xpu>::Launch(s,
                                      num_batch * topk,
                                      areas.dptr_,
                                      buffer.dptr_ + coord_start,
                                      sorted_index.dptr_,
                                      batch_start.dptr_,
                                      topk,
                                      num_elem,
...
void BoxNMSBackward(const nnvm::NodeAttrs& attrs,
  ...
  int width_elem         = in_shape[indim - 1];
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    Tensor<xpu, 3, DType> out_grad = inputs[box_nms_enum::kOut].get_with_shape<xpu, 3, DType>(
        Shape3(num_batch, num_elem, width_elem), s);
    Tensor<xpu, 3, DType> in_grad = outputs[box_nms_enum::kData].get_with_shape<xpu, 3, DType>(
        Shape3(num_batch, num_elem, width_elem), s);
...
inline bool BoxOverlapShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_GE(lshape.ndim(), 2) << "lhs must have dim >= 2 " << lshape.ndim() << " provided";
  int ldim = lshape[lshape.ndim() - 1];
  CHECK_EQ(ldim, 4) << "last dimension of lhs must be 4 " << ldim << " provided";
  CHECK_GE(rshape.ndim(), 2) << "rhs must have dim >= 2 " << rshape.ndim() << " provided";
  int rdim = rshape[rshape.ndim() - 1];
  CHECK_EQ(rdim, 4) << "last dimension of rhs must be 4 " << rdim << " provided";
...
void BoxOverlapForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    Tensor<xpu, 1, DType> lhs = inputs[0].get_with_shape<xpu, 1, DType>(Shape1(lsize * 4), s);
    Tensor<xpu, 1, DType> rhs = inputs[1].get_with_shape<xpu, 1, DType>(Shape1(rsize * 4), s);
    Tensor<xpu, 1, DType> out = outputs[0].get_with_shape<xpu, 1, DType>(Shape1(lsize * rsize), s);

    Kernel<compute_overlap, xpu>::Launch(
        s, lsize * rsize, out.dptr_, lhs.dptr_, rhs.dptr_, rsize, 0, 4, param.format);
  });
}
...
struct BipartiteMatchingParam : public dmlc::Parameter<BipartiteMatchingParam> {
  ...
  DMLC_DECLARE_PARAMETER(BipartiteMatchingParam) {
    ...
    DMLC_DECLARE_FIELD(threshold).describe(
        "Ignore matching when score < thresh, if is_ascend=false, "
...
inline bool MatchingShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_GE(dshape.ndim(), 2) << "score matrix must have dim >= 2 " << dshape.ndim() << " provided";
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    for (int j = 0; j < stride; ++j) {
      ...
      if (rmarker[r] == -1 && cmarker[c] == -1) {
        if ((!is_ascend && score[j] > threshold) || (is_ascend && score[j] < threshold)) {
          rmarker[r] = c;
          cmarker[c] = r;
          ++count;
...
void BipartiteMatchingForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    Tensor<xpu, 1, DType> scores =
        inputs[0].get_with_shape<xpu, 1, DType>(Shape1(dshape.Size()), s);
    Tensor<xpu, 2, DType> row_marker =
        outputs[0].get_with_shape<xpu, 2, DType>(Shape2(batch_size, row), s);
    Tensor<xpu, 2, DType> col_marker =
        outputs[1].get_with_shape<xpu, 2, DType>(Shape2(batch_size, col), s);
    ...
    Tensor<xpu, 1, DType> scores_copy(workspace.dptr_, sort_index_shape, s);
    Tensor<xpu, 1, int32_t> sorted_index(
        reinterpret_cast<int32_t*>(scores_copy.dptr_ + scores_copy.MSize()), sort_index_shape, s);
    Tensor<xpu, 1, int32_t> batch_id(
        sorted_index.dptr_ + sorted_index.MSize(), sort_index_shape, s);
    ...
    row_marker = -1;
    col_marker = -1;
    Kernel<bipartite_matching, xpu>::Launch(s,
                                            batch_size,
                                            row_marker.dptr_,
                                            col_marker.dptr_,
                                            scores_copy.dptr_,
...
inline bool BoxEncodeShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ(sshape.ndim(), 2) << "samples shape must have dim == 2, " << sshape.ndim()
                             << " provided";

  CHECK_GE(mshape.ndim(), 2) << "matches shape must have dim == 2, " << mshape.ndim()
                             << " provided";

  CHECK_GE(ashape.ndim(), 3) << "matches shape must have dim == 3, " << ashape.ndim()
                             << " provided";
  int ldim = ashape[ashape.ndim() - 1];
  CHECK_EQ(ldim, 4) << "last dimension of anchors must be 4, " << ldim << " provided";

  CHECK_GE(rshape.ndim(), 3) << "refs shape must have dim == 3, " << ashape.ndim() << " provided";
...
void BoxEncodeForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    Tensor<xpu, 2, DType> samples = inputs[0].get_with_shape<xpu, 2, DType>(Shape2(b, n), s);
    Tensor<xpu, 2, DType> matches = inputs[1].get_with_shape<xpu, 2, DType>(Shape2(b, n), s);
    Tensor<xpu, 3, DType> anchors = inputs[2].get_with_shape<xpu, 3, DType>(Shape3(b, n, 4), s);
    Tensor<xpu, 3, DType> refs    = inputs[3].get_with_shape<xpu, 3, DType>(Shape3(b, m, 4), s);
    Tensor<xpu, 1, DType> means   = inputs[4].get_with_shape<xpu, 1, DType>(Shape1(4), s);
    Tensor<xpu, 1, DType> stds    = inputs[5].get_with_shape<xpu, 1, DType>(Shape1(4), s);
    Tensor<xpu, 3, DType> out_targets =
        outputs[0].get_with_shape<xpu, 3, DType>(Shape3(b, n, 4), s);
    Tensor<xpu, 3, DType> out_masks = outputs[1].get_with_shape<xpu, 3, DType>(Shape3(b, n, 4), s);

    Kernel<box_encode, xpu>::Launch(s,
                                    loop_size,
                                    out_targets.dptr_,
                                    out_masks.dptr_,
                                    samples.dptr_,
...
struct BoxDecodeParam : public dmlc::Parameter<BoxDecodeParam> {
  ...
  DMLC_DECLARE_PARAMETER(BoxDecodeParam) {
    DMLC_DECLARE_FIELD(std0).set_default(1.0).describe(
        "value to be divided from the 1st encoded values");
    DMLC_DECLARE_FIELD(std1).set_default(1.0).describe(
        "value to be divided from the 2nd encoded values");
    DMLC_DECLARE_FIELD(std2).set_default(1.0).describe(
        "value to be divided from the 3rd encoded values");
    DMLC_DECLARE_FIELD(std3).set_default(1.0).describe(
        "value to be divided from the 4th encoded values");
    DMLC_DECLARE_FIELD(clip).set_default(-1.0).describe(
        "If larger than 0, bounding box target will be clipped to this value.");
    DMLC_DECLARE_FIELD(format)
        .set_default(box_common_enum::kCenter)
        .add_enum("corner", box_common_enum::kCorner)
        .add_enum("center", box_common_enum::kCenter)
        .describe(
            "The box encoding type. \n"
...
inline bool BoxDecodeShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ(dshape.ndim(), 3) << "data shape must have dim == 3, " << dshape.ndim() << " provided";
  int ldim = dshape[dshape.ndim() - 1];
  CHECK_EQ(ldim, 4) << "last dimension of data must be 4, " << ldim << " provided";

  CHECK_GE(ashape.ndim(), 3) << "anchors shape must have dim == 3, " << ashape.ndim()
                             << " provided";
...
void BoxDecodeForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    Tensor<xpu, 3, DType> data    = inputs[0].get_with_shape<xpu, 3, DType>(Shape3(b, n, 4), s);
    Tensor<xpu, 3, DType> anchors = inputs[1].get_with_shape<xpu, 3, DType>(Shape3(1, n, 4), s);
    Tensor<xpu, 3, DType> out     = outputs[0].get_with_shape<xpu, 3, DType>(Shape3(b, n, 4), s);
    if (box_common_enum::kCorner == param.format && param.clip > 0.0) {
      Kernel<box_decode<box_common_enum::kCorner, true>, xpu>::Launch(
          s,
          loop_size,
          out.dptr_,
          data.dptr_,
          anchors.dptr_,
          static_cast<DType>(param.std0),
          static_cast<DType>(param.std1),
          static_cast<DType>(param.std2),
          static_cast<DType>(param.std3),
          static_cast<DType>(param.clip),
          n);
    } else if (box_common_enum::kCenter == param.format && param.clip > 0.0) {
      Kernel<box_decode<box_common_enum::kCenter, true>, xpu>::Launch(
          s,
          loop_size,
          out.dptr_,
          data.dptr_,
      ...
      Kernel<box_decode<box_common_enum::kCorner, false>, xpu>::Launch(
          s,
          loop_size,
          out.dptr_,
          data.dptr_,
      ...
      Kernel<box_decode<box_common_enum::kCenter, false>, xpu>::Launch(
          s,
          loop_size,
          out.dptr_,
          data.dptr_,
