...
template <typename DType>
inline void DeformablePSROIPoolForwardCPU(const index_t count,
  ...
  for (index_t index = 0; index < count; index++) {
    ...
    for (index_t ih = 0; ih < sample_per_part; ih++) {
      for (index_t iw = 0; iw < sample_per_part; iw++) {
        ...
        DType val =
            bilinear_interp_cpu(offset_bottom_data + c * height * width, w, h, width, height);
        sum += val;
        count++;
      }
    }
...
template <typename DType>
inline void DeformablePSROIPoolBackwardAccCPU(const index_t count,
...
Operator* CreateOp<cpu>(DeformablePSROIPoolingParam param, int dtype) {
  Operator* op = nullptr;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, { op = new DeformablePSROIPoolingOp<cpu, DType>(param); });
  return op;
}
