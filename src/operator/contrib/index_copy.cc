...
void ValidateIndexCopyIndicesCPU(const TBlob& idx_vector, const index_t out_rows) {
  const IType* idx = idx_vector.dptr<IType>();
  for (index_t i = 0; i < idx_vector.Size(); ++i) {
...
void IndexCopyForward<cpu>(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_TYPE_SWITCH(out.type_flag_, DType, {
    MSHADOW_TYPE_SWITCH(idx_vector.type_flag_, IType, {
      ValidateIndexCopyIndicesCPU<IType>(idx_vector, original_tensor.shape_[0]);
...
void IndexCopyBackward<cpu>(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_TYPE_SWITCH(out_grad.type_flag_, DType, {
    MSHADOW_TYPE_SWITCH(index.type_flag_, IType, {
      ValidateIndexCopyIndicesCPU<IType>(index, out_grad.shape_[0]);
      switch (orig_req) {
        case kNullOp:
          break;
        case kWriteTo:
