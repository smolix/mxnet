...
inline int CUDA_GET_BLOCKS(const int N) {
  return std::max(std::min((N + kMaxThreadsPerBlock - 1) / kMaxThreadsPerBlock, MAXIMUM_NUM_BLOCKS),
...
template <typename T>
__device__ T bilinear_interpolate(const T* in_data,
                                  const int height,
                                  const int width,
                                  T y,
                                  T x,
...
template <typename T>
__device__ void RoIAlignForward(const T* in_data,  // (B, M, H, W)
                                const T* rois,     // (B, N, 4)
                                const T* matches,  // (B, N)
                                const int num_el,
  ...
  // Update kernel
  for (size_t index = blockIdx.x * blockDim.x + threadIdx.x; index < num_el;
    ...
    const T* offset_in_data =
        in_data + batch_idx * num_gtmasks * height * width + roi_batch_ind * height * width;
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {  // e.g., iy = 0, 1
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        T val = bilinear_interpolate(offset_in_data, height, width, y, x, index);
        output_val += val;
      }
    }
    output_val /= count;
...
template <>
void MRCNNMaskTargetRun<gpu>(const MRCNNMaskTargetParam& param,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    MRCNNMaskTargetKernel<<<dimGrid, dimBlock, 0, stream>>>(rois.dptr_,
                                                            gt_masks.dptr_,
                                                            matches.dptr_,
                                                            cls_targets.dptr_,
                                                            out_masks.dptr_,
