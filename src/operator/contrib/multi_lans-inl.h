...
/*!
 * \file multi_lans-inl.h
 * \brief multi-tensor LANS optimizer
 * \author Shuai Zheng
 */
#ifndef MXNET_OPERATOR_CONTRIB_MULTI_LANS_INL_H_
#define MXNET_OPERATOR_CONTRIB_MULTI_LANS_INL_H_
...
struct MultiLANSParam : public dmlc::Parameter<MultiLANSParam> {
  ...
  DMLC_DECLARE_PARAMETER(MultiLANSParam) {
    DMLC_DECLARE_FIELD(learning_rates).describe("List of learning rates");
    ...
    DMLC_DECLARE_FIELD(epsilon).set_default(1e-6f).describe("Small value to avoid division by 0.");
    DMLC_DECLARE_FIELD(wds).describe(
        "List of Weight decays."
        "Weight decay augments the objective function with a "
        "regularization term that penalizes large weights. "
        "The penalty scales with the square of the magnitude of each weight.");
    DMLC_DECLARE_FIELD(rescale_grad).set_default(1.0f).describe("Gradient rescaling factor");
    ...
    DMLC_DECLARE_FIELD(step_count).describe("Step count for each tensor");
    DMLC_DECLARE_FIELD(num_tensors).set_default(1).describe("Number of tensors");
  }
};

template <typename ParamType, int input_stride>
inline bool MultiLANSInferShape(const nnvm::NodeAttrs& attrs,
...
template <typename ParamType, int input_stride>
inline bool MPMultiLANSInferType(const nnvm::NodeAttrs& attrs,
  ...
  for (int i = 0; i < param.num_tensors; ++i) {
    ...
    all_inferred = all_inferred && ElemwiseType<2, 1>(attrs, &input_vec, &output_vec);
  }
...
template <typename T>
class LANSTypeIdentity {
 public:
  using type = T;
};

template <typename T>
class LANSSinglePrecision {
 public:
  using type = float;
};
...
template <typename xpu,
          typename DType,
          typename MPDType,
          typename ParamType = MultiLANSParam,
          int input_stride   = 5>
void FillMultiLANSKernelParam(const nnvm::NodeAttrs& attrs,
                              const OpContext& ctx,
                              const std::vector<TBlob>& inputs,
                              const std::vector<TBlob>& outputs,
                              MultiLANSKernelParam<DType, MPDType>* multi_param) {
  ...
  for (size_t i = 0; i < multi_param->ntensors; ++i) {
    ...
    multi_param->nchunks +=
        (multi_param->sizes[i] + multi_param->chunk_size - 1) / multi_param->chunk_size;
...
inline void MultiLANS(const nnvm::NodeAttrs& attrs,
                      const OpContext& ctx,
                      const std::vector<TBlob>& inputs,
                      const std::vector<OpReqType>& req,
                      const std::vector<TBlob>& outputs) {
  auto param     = nnvm::get<MultiLANSParam>(attrs.parsed);
  Stream<xpu>* s = ctx.get_stream<xpu>();

  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    using MPDType = typename MPTypeChooser<DType>::type;
    MultiLANSKernelParam<DType, MPDType> kernel_params;
    FillMultiLANSKernelParam<xpu, DType, MPDType, MultiLANSParam, input_stride>(
    ...
    // create vector of TBlob with all the weights contiguous
    std::vector<TBlob> grads;
    for (size_t index = 0; index < kernel_params.ntensors; ++index) {
      grads.emplace_back(inputs[index * input_stride + 1]);
    }

    // Calculate amount of temporary storage (temp_m, temp_g, r1, r2_m, r2_g, g_sq_norm,
    // block_to_tensor, block_to_chunk)
    size_t workspace_size = 2 * kernel_params.total_size * sizeof(float) +
                            4 * kernel_params.ntensors * sizeof(float) +
    ...
    Tensor<xpu, 1, char> workspace =
        ctx.requested[multilans::kTempSpace].get_space_typed<xpu, 1, char>(Shape1(workspace_size),
                                                                           s);

    // Create tensors
    size_t pos_wspace = required_storage_multi_sum_sq;
    Tensor<xpu, 1, float> temp_m(
        reinterpret_cast<float*>(&workspace[pos_wspace]), Shape1(kernel_params.total_size), s);
    // create vector of TBlob with all the temp_m and temp_g contiguous
    std::vector<TBlob> temp_m_tblobs;
    for (size_t index = 0; index < kernel_params.ntensors; ++index) {
      Tensor<xpu, 1, float> aux(
          reinterpret_cast<float*>(&workspace[pos_wspace]), Shape1(kernel_params.sizes[index]), s);
      TBlob newtblob(aux);
      temp_m_tblobs.emplace_back(newtblob);
      pos_wspace += kernel_params.sizes[index] * sizeof(float);
    }
    Tensor<xpu, 1, float> temp_g(
        reinterpret_cast<float*>(&workspace[pos_wspace]), Shape1(kernel_params.total_size), s);
    ...
    Tensor<xpu, 1, float> r1(
        reinterpret_cast<float*>(&workspace[pos_wspace]), Shape1(kernel_params.ntensors), s);
    pos_wspace += kernel_params.ntensors * sizeof(float);
    Tensor<xpu, 1, float> r2_m(
        reinterpret_cast<float*>(&workspace[pos_wspace]), Shape1(kernel_params.ntensors), s);
    pos_wspace += kernel_params.ntensors * sizeof(float);
    Tensor<xpu, 1, float> r2_g(
        reinterpret_cast<float*>(&workspace[pos_wspace]), Shape1(kernel_params.ntensors), s);
    pos_wspace += kernel_params.ntensors * sizeof(float);
    Tensor<xpu, 1, float> g_sq_norm(
        reinterpret_cast<float*>(&workspace[pos_wspace]), Shape1(kernel_params.ntensors), s);
    pos_wspace += kernel_params.ntensors * sizeof(float);
    Tensor<xpu, 1, int> block_to_tensor(
        reinterpret_cast<int*>(&workspace[pos_wspace]), Shape1(kernel_params.nchunks), s);
    pos_wspace += kernel_params.nchunks * sizeof(int);
    Tensor<xpu, 1, int> block_to_chunk(
        reinterpret_cast<int*>(&workspace[pos_wspace]), Shape1(kernel_params.nchunks), s);
    ...
    MultiSumSqRun<xpu>(temp_m_tblobs, kernel_params.ntensors, r2_m.dptr_, ctx);
    MultiSumSqRun<xpu>(temp_g_tblobs, kernel_params.ntensors, r2_g.dptr_, ctx);
    CallKernel2<MPDType, DType>(s,
                                kernel_params,
                                param,
                                r1.dptr_,
                                r2_m.dptr_,
...
inline void MultiLANSUpdate(const nnvm::NodeAttrs& attrs,
                            const OpContext& ctx,
                            const std::vector<TBlob>& inputs,
                            const std::vector<OpReqType>& req,
                            const std::vector<TBlob>& outputs) {
  if (!MP) {
    MultiLANS<xpu, LANSTypeIdentity, 4>(attrs, ctx, inputs, req, outputs);
  } else {
    MultiLANS<xpu, LANSSinglePrecision, 5>(attrs, ctx, inputs, req, outputs);
  }
}

}  // namespace op
}  // namespace mxnet
#endif  // MXNET_OPERATOR_CONTRIB_MULTI_LANS_INL_H_
