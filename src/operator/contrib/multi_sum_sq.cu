...
template <typename DType>
__device__ __forceinline__ DType ReduceBlockIntoLanes(DType* x, DType val) {
  int tid        = threadIdx.x;
...
__global__ void MultiSumSqKernel(int chunk_size,
  ...
  float val = 0;
  for (int i_start = 0; i_start < i_max; i_start += blockDim.x * ILP) {
...
template <>
void MultiSumSqRun<gpu>(const std::vector<TBlob>& inputs,
                        int n_inputs,
                        float* out_ptr,
                        const OpContext& ctx,
                        float scale) {
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    MultiSumSqKernelParam<DType> param;
    size_t workspace_size = GetRequiredStorageMultiSumSq<gpu>(inputs, &param.max_chunks_per_tensor);
    Tensor<gpu, 1, char> workspace =
        ctx.requested[multi_sum_sq::kTempSpace].get_space_typed<gpu, 1, char>(
            Shape1(workspace_size), s);
    Tensor<gpu, 1, float> block_reductions(
        reinterpret_cast<float*>(&workspace[0]), Shape1(n_inputs * param.max_chunks_per_tensor), s);
    CUDA_CALL(cudaMemsetAsync(
        block_reductions.dptr_, 0, n_inputs * param.max_chunks_per_tensor * sizeof(float), stream));
...
NNVM_REGISTER_OP(multi_sum_sq).set_attr<FCompute>("FCompute<gpu>", MultiSumSq<gpu>);
