...
__global__ void InitGroundTruthFlags(DType* gt_flags,
  ...
  if (index >= num_batches * num_labels)
    return;
...
template <typename DType>
__global__ void FindBestMatches(DType* best_matches,
                                DType* gt_flags,
                                DType* anchor_flags,
                                const DType* overlaps,
  ...
  while (1) {
    ...
    if (threadIdx.x == 0) {
      ...
      for (int k = 0; k < num_threads; ++k) {
        if (max_indices_y[k] < 0 || max_indices_x[k] < 0)
          continue;
...
template <typename DType>
__global__ void FindGoodMatches(DType* best_matches,
                                DType* anchor_flags,
                                const DType* overlaps,
                                const int num_anchors,
                                const int num_labels,
...
template <typename DType>
__global__ void NegativeMining(const DType* overlaps,
                               const DType* cls_preds,
                               DType* anchor_flags,
                               DType* buffer,
  ...
  for (int width = 2; width < (num_anchors << 1); width <<= 1) {
    ...
    for (int slice = 0; slice < slices; ++slice) {
      if (start >= num_anchors)
        break;
      int middle = start + (width >> 1);
      if (num_anchors < middle)
        middle = num_anchors;
      int end = start + width;
      if (num_anchors < end)
        end = num_anchors;
...
template <typename DType>
__global__ void AssignTrainigTargets(DType* loc_target,
                                     DType* loc_mask,
                                     DType* cls_target,
                                     DType* anchor_flags,
                                     DType* best_matches,
...
Operator* CreateOp<gpu>(MultiBoxTargetParam param, int dtype) {
  Operator* op = nullptr;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, { op = new MultiBoxTargetOp<gpu, DType>(param); });
  return op;
}
}  // namespace op
}  // namespace mxnet
