...
class ProposalOp : public Operator {
 public:
  ...
  void Forward(const OpContext& ctx,
    ...
    if (param_.iou_loss) {
      utils::IoUTransformInv(workspace_proposals,
                             bbox_deltas,
                             im_info[0][0],
                             im_info[0][1],
                             real_height,
      ...
      utils::BBoxTransformInv(workspace_proposals,
                              bbox_deltas,
                              im_info[0][0],
                              im_info[0][1],
                              real_height,
    ...
    utils::CopyScore(workspace_proposals, &score, &order);
    utils::ReverseArgsort(score, &order);
    utils::ReorderProposals(
        workspace_proposals, order, rpn_pre_nms_top_n, &workspace_ordered_proposals);
