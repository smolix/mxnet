      ...
      for (int iy = 0; iy < iy_upper; iy++) {
        ...
        for (int ix = 0; ix < ix_upper; ix++) {
          const DType xx =
              ...
              static_cast<DType>(ix + .5f) * bin_size_w / static_cast<DType>(roi_bin_grid_w);
...
template <typename DType>
inline void RROIAlignForward(const OpContext& ctx,
                             const RROIAlignParam& param,
...
#pragma omp parallel for num_threads(engine::OpenMP::Get()->GetRecommendedOMPThreadCount())
  for (int n = 0; n < num_rois; ++n) {
    ...
    pre_calc_for_bilinear_interpolate(height_,
                                      width_,
                                      pooled_height_,
                                      pooled_width_,
                                      roi_bin_grid_h,
