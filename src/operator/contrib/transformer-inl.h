...
struct SldWinAttenParam : public dmlc::Parameter<SldWinAttenParam> {
  ...
  DMLC_DECLARE_PARAMETER(SldWinAttenParam) {
    DMLC_DECLARE_FIELD(w).describe("The one-sided window length");
    DMLC_DECLARE_FIELD(symmetric).describe(
...
struct SldWinAttenMaskLike {
  MSHADOW_XINLINE static void Map(int i,
    ...
    bool is_zero = idx_3 < (w - idx_1 / dilation[idx_2]) || idx_1 >= val_length[idx_0] ||
                   (symmetric &&
                    (w_len - idx_3 - 1) < (w - (val_length[idx_0] - idx_1 - 1) / dilation[idx_2]));
    if (is_zero)
      out[i] = 0;
  }
};
...
struct DiagMM {
  MSHADOW_XINLINE static void Map(int tid,
    ...
    if (!diagonal_lhs) {
      ...
      if (tmp_idx >= seq_length || tmp_idx < 0)
        return;
      ...
      if (!transpose_lhs) {
        ...
        for (int i = 0; i < lhs_last_dim; i++) {
          ...
          if (tmp_idx >= seq_length || tmp_idx < 0)
            continue;
          int lhs_idx = idx_0 * (seq_length * num_heads * lhs_last_dim) +
                        tmp_idx * (num_heads * lhs_last_dim) + idx_2 * lhs_last_dim +
                        ((w_right + w) - i);
          int rhs_idx = idx_0 * (seq_length * num_heads * out_last_dim) +
                        tmp_idx * (num_heads * out_last_dim) + idx_2 * out_last_dim + idx_3;
          out[tid] += lhs[lhs_idx] * rhs[rhs_idx];
...
template <typename xpu>
void DiagMMImpl(const OpContext& ctx,
                const TBlob& out,
                const TBlob& lhs,
                const TBlob& rhs,
                const TBlob& dilation,
                bool diagonal_lhs,
                bool transpose_lhs,
                int w,
                int w_right) {
...
void SldWinAttenScoreForward(const nnvm::NodeAttrs& attrs,
  ...
  DiagMMImpl<xpu>(
      ctx, outputs.at(0), inputs.at(0), inputs.at(1), inputs.at(2), false, false, param.w, w_right);
}
...
void SldWinAttenScoreBackward(const nnvm::NodeAttrs& attrs,
  ...
  DiagMMImpl<xpu>(
      ctx, outputs.at(0), inputs.at(0), inputs.at(2), inputs.at(3), true, false, param.w, w_right);
  // grad_key = matmul(grad_score.T, query)
  DiagMMImpl<xpu>(
      ctx, outputs.at(1), inputs.at(0), inputs.at(1), inputs.at(3), true, true, param.w, w_right);
}
...
void SldWinAttenContextForward(const nnvm::NodeAttrs& attrs,
  ...
  DiagMMImpl<xpu>(
      ctx, outputs.at(0), inputs.at(0), inputs.at(1), inputs.at(2), true, false, param.w, w_right);
}
...
void SldWinAttenContextBackward(const nnvm::NodeAttrs& attrs,
  ...
  DiagMMImpl<xpu>(
      ctx, outputs.at(0), inputs.at(0), inputs.at(2), inputs.at(3), false, false, param.w, w_right);
  // grad_value = matmul(score.T, grad_context)
  DiagMMImpl<xpu>(
      ctx, outputs.at(1), inputs.at(1), inputs.at(0), inputs.at(3), true, true, param.w, w_right);
}
