...
struct CropParam : public dmlc::Parameter<CropParam> {
  ...
  DMLC_DECLARE_PARAMETER(CropParam) {
    DMLC_DECLARE_FIELD(x).describe("Left boundary of the cropping area.");
    DMLC_DECLARE_FIELD(y).describe("Top boundary of the cropping area.");
    DMLC_DECLARE_FIELD(width).describe("Width of the cropping area.");
    DMLC_DECLARE_FIELD(height).describe("Height of the cropping area.");
  }
};
...
inline bool CropShape(const nnvm::NodeAttrs& attrs,
  ...
  if (in_attrs->at(0).ndim() == 3U) {
    CHECK((in_attrs->at(0)[2] == 1) || (in_attrs->at(0)[2] == 3))
        << "Expect channel of the input image is 1 or 3, but got" << in_attrs->at(0)[2];
  } else if (in_attrs->at(0).ndim() == 4U) {
    CHECK((in_attrs->at(0)[3] == 1) || (in_attrs->at(0)[3] == 3))
        << "Expect channel of the input image is 1 or 3, but got" << in_attrs->at(0)[3];
  } else {
  ...
  CHECK((param.height > 0) && (param.width > 0)) << "Input height and width must be greater than 0";
...
inline void CropBackwardImpl(int x,
  ...
  MXNET_NDIM_SWITCH(output_grad.ndim(), ndim, {
    ...
    MSHADOW_TYPE_SWITCH(output_grad.type_flag_, DType, {MXNET_ASSIGN_REQ_SWITCH(req[0], Req, {
                          ...
                          mxnet_op::Kernel<slice_assign<ndim, Req, xpu>, xpu>::Launch(
                              ...
                              input_grad.shape_.get<ndim>(),
                              output_grad.shape_.get<ndim>(),
                              begin,
                              step);
                        })})
  })
...
struct RandomCropParam : public dmlc::Parameter<RandomCropParam> {
  ...
  DMLC_DECLARE_PARAMETER(RandomCropParam) {
    DMLC_DECLARE_FIELD(xrange)
        .set_default(Tuple<float>({0.f, 1.f}))
        .describe("Left boundaries of the cropping area.");
    DMLC_DECLARE_FIELD(yrange)
        .set_default(Tuple<float>({0.f, 1.f}))
        .describe("Top boundaries of the cropping area.");
    DMLC_DECLARE_FIELD(width).describe("The target image width");
    DMLC_DECLARE_FIELD(height).describe("The target image height.");
...
inline bool RandomCropShape(const nnvm::NodeAttrs& attrs,
  ...
  if (ishape.ndim() == 3) {
    SHAPE_ASSIGN_CHECK(*out_attrs, 1, TShape({scaled_shape[1], scaled_shape[0], ishape[C]}));
  } else {
    SHAPE_ASSIGN_CHECK(
        *out_attrs, 1, TShape({ishape[N], scaled_shape[1], scaled_shape[0], ishape[kC]}));
  }
  return true;
}
...
inline void RandomCropOpForward(const nnvm::NodeAttrs& attrs,
  ...
  float x = std::uniform_real_distribution<float>(param.xrange[0],
            ...
            (src_size[0] - resize_size[0]);
  float y = std::uniform_real_distribution<float>(param.yrange[0],
                                                  param.yrange[1])(prnd->GetRndEngine()) *
            (src_size[1] - resize_size[1]);
  // write x, y, w, h to temp workspace
  Tensor<cpu, 1> workspace = ctx.requested[1].get_space<cpu>(mshadow::Shape1(4), s);
  workspace.dptr_[0]       = x;
  workspace.dptr_[1]       = y;
...
inline void RandomCropOpBackward(const nnvm::NodeAttrs& attrs,
  ...
  Tensor<cpu, 1> workspace =
      ctx.requested[1].get_space<cpu>(mshadow::Shape1(4), ctx.get_stream<cpu>());
...
struct RandomResizedCropParam : public dmlc::Parameter<RandomResizedCropParam> {
  ...
  DMLC_DECLARE_PARAMETER(RandomResizedCropParam) {
    ...
    DMLC_DECLARE_FIELD(area)
        .set_default(Tuple<float>({0.08f, 1.f}))
        .describe("Range of cropping area percentage.");
    DMLC_DECLARE_FIELD(ratio)
        .set_default(Tuple<float>({3 / 4.f, 4 / 3.f}))
    ...
    DMLC_DECLARE_FIELD(interp).set_default(1).describe(
        ...
        "INTER_LANCZOS4 - a Lanczos interpolation over 8x8 pixel neighborhood"
        "Note that the GPU version only support bilinear interpolation(1)");
    DMLC_DECLARE_FIELD(max_trial).set_default(10).describe(
        "Max trial before fallback to center crop.");
  }
};
...
inline void RandomResizedCropOpForward(const nnvm::NodeAttrs& attrs,
  ...
  for (int i = 0; i < param.max_trial; ++i) {
    ...
    float new_ratio      = std::exp(
        std::uniform_real_distribution<float>(log_ratio_low, log_ratio_high)(prnd->GetRndEngine()));
    ...
    if (new_w <= src_size[0] && new_h <= src_size[1]) {
      int x0 =
          std::uniform_real_distribution<float>(0, 1)(prnd->GetRndEngine()) * (src_size[0] - new_w);
      int y0 =
          std::uniform_real_distribution<float>(0, 1)(prnd->GetRndEngine()) * (src_size[1] - new_h);
      if (new_w == param.width && new_h == param.height) {
        // no need to resize
        CropImpl<xpu>(x0, y0, new_w, new_h, inputs, outputs, ctx, req);
      } else {
        CropResizeImpl<xpu>(ctx,
                            inputs,
                            req,
                            outputs,
                            x0,
