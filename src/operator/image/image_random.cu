  ...
  for (int c = 0; c < C; ++c) {
    for (int h = threadIdx.y; h < H; h += blockDim.y) {
      for (int w = threadIdx.x; w < W; w += blockDim.x) {
        KERNEL_ASSIGN(output[c][h][w], req, input[h][w][c] / normalize_factor);
      }
    }
  }
}
...
__global__ void __launch_bounds__(mshadow::cuda_impl::kMaxThreadsPerBlock, 1)
  ...
  for (int c = 0; c < C; ++c) {
    for (int h = threadIdx.y; h < H; h += blockDim.y) {
      for (int w = threadIdx.x; w < W; w += blockDim.x) {
        KERNEL_ASSIGN(output[n][c][h][w], req, input[n][h][w][c] / normalize_factor);
      }
    }
  }
}
...
void ToTensorImplCUDA(mshadow::Stream<gpu>* s,
  ...
  ToTensorCudaKernel<gpu, DType>
      <<<blocks, dim3(32, 32), 0, stream>>>(input, output, req, N, H, W, C, normalize_factor);
...
__global__ void __launch_bounds__(mshadow::cuda_impl::kMaxThreadsPerBlock, 1)
  ...
  for (int c = 0; c < C; ++c) {
    ...
    for (int i = threadIdx.x; i < length; i += blockDim.x) {
      KERNEL_ASSIGN(*(output + step + i + (c * length)),
                    req,
...
__global__ void __launch_bounds__(mshadow::cuda_impl::kMaxThreadsPerBlock, 1)
  ...
  for (int c = 0; c < C; ++c) {
    ...
    for (int i = threadIdx.x; i < length; i += blockDim.x) {
      KERNEL_ASSIGN(*(in_grad + step + i + (c * length)),
                    req,
...
NNVM_REGISTER_OP(_image_to_tensor).set_attr<FCompute>("FCompute<gpu>", ToTensorOpForward<gpu>);

NNVM_REGISTER_OP(_image_normalize).set_attr<FCompute>("FCompute<gpu>", NormalizeOpForward<gpu>);
