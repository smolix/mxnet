...
void EigOpForwardImpl(const TBlob& a,
  ...
  MSHADOW_SGL_DBL_TYPE_SWITCH(w.type_flag_, DType, {
    ...
    DType* vr_ptr   = reinterpret_cast<DType*>(workspace->data() +
                                             (2 * w.Size() + N * N + a.Size()) * sizeof(DType));
    DType* work_ptr = reinterpret_cast<DType*>(
...
void GpuCallbackCpuImpl(const TBlob& a,
  ...
  CUDA_CALL(cudaMemcpyAsync(
      v_cp_ptr, v.dptr<WType>(), sizeof(WType) * v.Size(), cudaMemcpyDeviceToHost, stream));
  ...
  CUDA_CALL(cudaMemcpyAsync(
      v.dptr<WType>(), v_cp_ptr, sizeof(WType) * v.Size(), cudaMemcpyHostToDevice, stream));
  CUDA_CALL(cudaStreamSynchronize(stream));
#else
