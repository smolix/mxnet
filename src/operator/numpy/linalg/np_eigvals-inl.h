...
#define LINALG_CPU_EIG(fname, DType)                                                       \
  template <>                                                                              \
  inline void linalg_geev<cpu, DType>(char jobvl,                                          \
                                      char jobvr,                                          \
    ...
    int res(MXNET_LAPACK_##fname(MXNET_LAPACK_COL_MAJOR,                                   \
                                 jobvl,                                                    \
                                 jobvr,                                                    \
                                 n,                                                        \
                                 a.dptr_,                                                  \
...
#define LINALG_GPU_EIG(fname, DType)                                             \
  template <>                                                                    \
  inline void linalg_geev<gpu, DType>(char jobvl,                                \
                                      char jobvr,                                \
...
void EigvalsOpForwardImpl(const TBlob& a,
  ...
  MSHADOW_SGL_DBL_TYPE_SWITCH(w.type_flag_, DType, {
    ...
    DType* wr_ptr = reinterpret_cast<DType*>(workspace->data() + a.Size() * sizeof(DType));
    DType* wi_ptr =
    ...
    DType* vr_ptr   = reinterpret_cast<DType*>(workspace->data() +
                                             (N * N + 2 * w.Size() + a.Size()) * sizeof(DType));
    DType* work_ptr = reinterpret_cast<DType*>(workspace->data() +
                                               (2 * (N * N + w.Size()) + a.Size()) * sizeof(DType));
    ...
    for (size_t i = 0; i < wi_data.Size(); ++i) {
      CHECK_LE(fabs(wi_ptr[i]), 1e-15) << "Complex eigvals is unsupported in linalg temporary.";
    }
...
void GpuCallbackCpuImpl(const TBlob& a,
  ...
  CUDA_CALL(cudaMemcpyAsync(
      a_cp_ptr, a.dptr<AType>(), sizeof(AType) * a.Size(), cudaMemcpyDeviceToHost, stream));
  CUDA_CALL(cudaMemcpyAsync(
      w_cp_ptr, w.dptr<WType>(), sizeof(WType) * w.Size(), cudaMemcpyDeviceToHost, stream));
  ...
  // Copy back to gpu.
  CUDA_CALL(cudaMemcpyAsync(
      w.dptr<WType>(), w_cp_ptr, sizeof(WType) * w.Size(), cudaMemcpyHostToDevice, stream));
  CUDA_CALL(cudaStreamSynchronize(stream));
#else
...
struct EigvalshParam : public dmlc::Parameter<EigvalshParam> {
  ...
  DMLC_DECLARE_PARAMETER(EigvalshParam) {
    DMLC_DECLARE_FIELD(UPLO).set_default('L').describe(
  ...
  MSHADOW_TYPE_SWITCH(a.type_flag_, AType, {
    MSHADOW_SGL_DBL_TYPE_SWITCH(w.type_flag_, WType, {
      ...
      work_space_size +=            // For workspace size in LAPACKE_#SYEVD.
          linalg_syevd_workspace_query(
              a_data.get<cpu, 2, WType>(s_cpu), w_data.get<cpu, 1, WType>(s_cpu), s_cpu);
...
void EigvalshOpForwardImpl(const TBlob& a,
  ...
  MSHADOW_TYPE_SWITCH(a.type_flag_, AType, {
    MSHADOW_SGL_DBL_TYPE_SWITCH(w.type_flag_, DType, {
      ...
      DType* a_ptr                = reinterpret_cast<DType*>(workspace->data());
      DType* w_ptr = reinterpret_cast<DType*>(workspace->data() + a.Size() * sizeof(DType));
      DType* work_ptr =
      ...
      TBlob work_data(
          work_ptr, Shape1(workspace_size - a.Size() - w.Size()), w.dev_mask(), w.dev_id());
      ...
      mxnet_op::Kernel<eigh_eigvalsh_helper, xpu>::Launch(s,
                                                          a.Size(),
                                                          a.dptr<AType>(),
                                                          A.dptr_,
                                                          A.size(1),
