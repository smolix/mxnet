...
template <typename xpu, typename DType>
int linalg_qr_workspace_query(const Tensor<xpu, 2, DType>& A, Stream<xpu>* s = 0);
...
template <typename xpu, typename DType>
inline void check_geqrf(const Tensor<xpu, 2, DType>& A, const Tensor<xpu, 1, DType>& work) {
...
#define LINALG_CPU_GEQRF(fname, DType)                                                          \
  template <>                                                                                   \
  inline void linalg_geqrf<cpu, DType>(                                                         \
    ...
    int ret(MXNET_LAPACK_##fname(                                                               \
        MXNET_LAPACK_COL_MAJOR, m, n, A.dptr_, A.stride_, work.dptr_, work.dptr_ + mn, lwork)); \
...
#define LINALG_CPU_ORGQR(fname, DType)                                                     \
  template <>                                                                              \
  inline void linalg_orgqr<cpu, DType>(                                                    \
      const Tensor<cpu, 2, DType>& A, const Tensor<cpu, 1, DType>& work, Stream<cpu>* s) { \
    check_geqrf(A, work);                                                                  \
    const int m  = A.size(1);                                                              \
    const int n  = A.size(0);                                                              \
...
#define LINALG_CPU_QR_WORKSPACE_QUERY(prefix, DType)                               \
  template <>                                                                      \
  inline int linalg_qr_workspace_query<cpu, DType>(const Tensor<cpu, 2, DType>& A, \
                                                   Stream<cpu>* s) {               \
    const int m(A.size(1));                                                        \
    const int n(A.size(0));                                                        \
    const int mn = (m > n ? n : m);                                                \
    DType work   = 0;                                                              \
    int ret(MXNET_LAPACK_##prefix##geqrf(                                          \
        MXNET_LAPACK_COL_MAJOR, m, n, A.dptr_, A.stride_, &work, &work, -1));      \
    CHECK_EQ(ret, 0) << #prefix << "geqrf: Workspace query failed on CPU.";        \
    int ws_size(static_cast<int>(work));                                           \
    ret = MXNET_LAPACK_##prefix##orgqr(                                            \
        MXNET_LAPACK_COL_MAJOR, m, mn, mn, A.dptr_, A.stride_, &work, &work, -1);  \
...
#define LINALG_GPU_GEQRF(fname, DType)                                                     \
  template <>                                                                              \
  inline void linalg_geqrf<gpu, DType>(                                                    \
...
#define LINALG_GPU_ORGQR(fname, DType)                                                     \
  template <>                                                                              \
  inline void linalg_orgqr<gpu, DType>(                                                    \
      const Tensor<gpu, 2, DType>& A, const Tensor<gpu, 1, DType>& work, Stream<gpu>* s) { \
    using namespace mxnet;                                                                 \
    using mshadow::gpu;                                                                    \
    CHECK_NOTNULL(s);                                                                      \
...
#define LINALG_GPU_QR_WORKSPACE_QUERY(prefix, DType)                                    \
  template <>                                                                           \
  inline int linalg_qr_workspace_query<gpu, DType>(const Tensor<gpu, 2, DType>& A,      \
                                                   Stream<gpu>* s) {                    \
    using namespace mxnet;                                                              \
    using mshadow::gpu;                                                                 \
    const int m(A.size(1));                                                             \
    ...
    CUSOLVER_CALL(cusolverDn##prefix##geqrf_bufferSize(                                 \
        Stream<gpu>::GetSolverHandle(s), m, n, A.dptr_, A.stride_, &work1));            \
    int work2(0);                                                                       \
  ...
  MSHADOW_TYPE_SWITCH(a.type_flag_, AType, {
    MSHADOW_SGL_DBL_TYPE_SWITCH(q.type_flag_, DType, {
      ...
      if (xpu::kDevCPU) {
        ...
        work_space_size += linalg_qr_workspace_query(a_data.get<xpu, 2, DType>(s), s);
      } else {
        Storage::Handle a_handle =
            Storage::Get()->Alloc(sizeof(DType) * a_temp_tensor.shape_.Size(), Context::GPU());
        TBlob a_data(
            static_cast<DType*>(a_handle.dptr), a_temp_tensor.shape_, a.dev_mask(), a.dev_id());
        work_space_size += linalg_qr_workspace_query(a_data.get<xpu, 2, DType>(s), s);
...
void QrOpForwardImpl(const TBlob& a,
  ...
  MSHADOW_SGL_DBL_TYPE_SWITCH(q.type_flag_, DType, {
    ...
    if (m == n) {
      ...
      TBlob work_data(work_ptr, Shape1(workspace_size - a_shape.Size()), a.dev_mask(), a.dev_id());
      Tensor<xpu, 1, DType> Work = work_data.get<xpu, 1, DType>(s);
      qr::op(At, Qt, Work, ctx, attrs);
      // Transpose into R
      mxnet_op::Kernel<QrTypeTransposeHelper, xpu>::Launch(
      ...
      TBlob work_data(
          work_ptr, Shape1(workspace_size - 2 * a_shape.Size()), a.dev_mask(), a.dev_id());
...
struct QrBackHelper_G1 {
  template <typename DType>
  MSHADOW_XINLINE static void Map(const int k,
                                  const int m,
                                  const int n,
                                  const DType* in_data,
                                  const int ldin,
...
struct QrBackHelper_G2 {
  ...
  MSHADOW_XINLINE static void Map(const int k,
                                  const int m,
                                  const int n,
                                  const DType* in_data_x,
                                  const int ldinx,
                                  const DType* in_data_y,
                                  const int ldiny,
                                  DType* out_data,
                                  const int ldout) {
...
void QrBackwardImpl(const TBlob& grad_a,
  ...
  MSHADOW_SGL_DBL_TYPE_SWITCH(grad_a.type_flag_, DType, {
    ...
    if (m >= n) {
      ...
      // dX = op call
      qr_backward::op(dX, dQ_prime, dU, Q, U, M, ctx, attrs);
