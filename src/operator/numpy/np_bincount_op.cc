...
void BinNumberCount(const NDArray& data,
                    const int& minlength,
                    const NDArray& out,
                    const size_t& N) {
  int bin = minlength;
...
void BincountCpuWeights(const DType* data, const OType* weights, OType* out, const size_t& data_n) {
...
void NumpyBincountForwardImpl<cpu>(const OpContext& ctx,
  ...
  MXNET_INT_TYPE_SWITCH_EXT_WITH_BOOL(data.dtype(), DType, {
    MSHADOW_TYPE_SWITCH(weights.dtype(), OType, {
      ...
      BincountCpuWeights(data.data().dptr<DType>(),
...
NNVM_REGISTER_OP(_npi_bincount)
    ...
    .set_num_inputs([](const NodeAttrs& attrs) {
      const NumpyBincountParam& params = nnvm::get<NumpyBincountParam>(attrs.parsed);
