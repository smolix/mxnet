...
namespace mxnet_op {
struct set_to_nan {
  MSHADOW_XINLINE static float Map() {
    return nanf("");
  }
...
void NumpyReduceAxesBoolCompute(const nnvm::NodeAttrs& attrs,
  ...
  if (inputs[0].shape_.Size() == 0 && outputs[0].shape_.Size() != 0) {
    ...
    MXNET_ASSIGN_REQ_SWITCH(req[0], Req, {
    if (init == 0) {
      Kernel<op_with_req<set_false, Req>, xpu>::Launch(
          s, outputs[0].shape_.Size(), outputs[0].dptr<bool>());
    } else {
      Kernel<op_with_req<set_true, Req>, xpu>::Launch(
          s, outputs[0].shape_.Size(), outputs[0].dptr<bool>());
    }
    });
    return;
