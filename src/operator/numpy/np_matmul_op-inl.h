...
inline bool MatmulNeedBroadcast(const mxnet::TShape& ashape, const mxnet::TShape& bshape) {
  bool need_bcast = false;
  for (int i = ashape.ndim() - 3, j = bshape.ndim() - 3; i >= 0 || j >= 0; --i, --j) {
    if (i >= 0 && j >= 0) {
...
mshadow::Shape<ndim> GetBroadcastKernelShape(mshadow::Shape<ndim> in_shape,
  ...
  CHECK_LT(dimend, ndim) << "dimend must be < " << ndim << ", while received " << dimend;
...
inline void MatmulImpl(const OpContext& ctx,
                       ...
                       const size_t bc_size_a,
                       const size_t bc_size_b,
      ...
      MSHADOW_TYPE_SWITCH_WITH_BOOL(input_b.type_flag_, OType, {
        ...
        if (isCPU) {
          if (!aux_data_a.shape_changed) {
            ...
            Kernel<broadcast_kernel_cpu<mshadow_op::identity>, xpu>::Launch(s,
                                                                            input_b.Size(),
                                                                            input_b.dptr<IType>(),
                                                                            bc_b_ptr,
                                                                            aux_data_b,
          ...
          Kernel<broadcast_kernel_gpu<mshadow_op::identity>, xpu>::Launch(
              s, bc_size_a, input_a.dptr<IType>(), bc_a_ptr, aux_data_a, OpReqType::kWriteTo, ndim);
          Kernel<broadcast_kernel_gpu<mshadow_op::identity>, xpu>::Launch(
              s, bc_size_b, input_b.dptr<IType>(), bc_b_ptr, aux_data_b, OpReqType::kWriteTo, ndim);
        }
      });
    });
    ans = mshadow::Tensor<xpu, 3, DType>(
        output.dptr<DType>(), Shape3(batch_size, k_out_shape[ndim - 2], k_out_shape[ndim - 1]), s);
    mlhs = mshadow::Tensor<xpu, 3, DType>(
        bc_a_ptr, Shape3(batch_size, k_a_shape_bc[ndim - 2], k_a_shape_bc[ndim - 1]), s);
    mrhs = mshadow::Tensor<xpu, 3, DType>(
        bc_b_ptr, Shape3(batch_size, k_b_shape_bc[ndim - 2], k_b_shape_bc[ndim - 1]), s);
...
void NumpyMatmulForward(const nnvm::NodeAttrs& attrs,
  ...
  mshadow::Stream<xpu>* s = ctx.get_stream<xpu>();
  CHECK_NE(a.shape_.ndim(), 0) << "Multiplication by scalars is not allowed.\n";
  CHECK_NE(b.shape_.ndim(), 0) << "Multiplication by scalars is not allowed.\n";
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    ...
    size_t temp_mem_size =
        (bc_size_a + bc_size_b) * sizeof(DType) + 3 * batch_size * sizeof(DType*);
    Tensor<xpu, 1, char> temp_mem =
        ctx.requested[0].get_space_typed<xpu, 1, char>(Shape1(temp_mem_size), s);
    MatmulImpl<xpu, DType>(ctx,
                           inputs[0],
                           inputs[1],
                           req[0],
                           outputs[0],
...
void NumpyMatmulBackward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(ograd.type_flag_, DType, {
    ...
    size_t temp_mem_size_grada =
        (bc_size_out + bc_size_b) * sizeof(DType) + 3 * batch_size * sizeof(DType*);
    size_t temp_mem_size_gradb =
        (bc_size_a + bc_size_out) * sizeof(DType) + 3 * batch_size * sizeof(DType*);
    size_t temp_size_grada = bc_size_a * sizeof(DType);
    size_t temp_size_gradb = bc_size_b * sizeof(DType);
    size_t temp_mem_size =
        temp_mem_size_grada + temp_mem_size_gradb + temp_size_grada + temp_size_gradb;
    Tensor<xpu, 1, char> temp_mem =
        ctx.requested[0].get_space_typed<xpu, 1, char>(Shape1(temp_mem_size), s);
    Tensor<xpu, 1, char> workspace_grada(temp_mem.dptr_, Shape1(temp_mem_size_grada), s);
    Tensor<xpu, 1, char> workspace_gradb(
        workspace_grada.dptr_ + temp_mem_size_grada, Shape1(temp_mem_size_gradb), s);
    ...
    Tensor<xpu, 1, DType> temp_gradb(
        reinterpret_cast<DType*>(temp_grada.dptr_ + bc_size_a), Shape1(bc_size_b), s);
    MatmulImpl<xpu, DType>(ctx,
                           ograd,
                           b,
                           kWriteTo,
    ...
    MatmulImpl<xpu, DType>(ctx,
                           ...
                           true,
                           false);
    Kernel<SumByShape, xpu>::Launch(s,
                                    a_shape.Size(),
                                    grad_a.dptr<DType>(),
                                    temp_grada.dptr_,
                                    bc_size_a,
