...
inline bool NumpyMatmulShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_NE(a_ndim, 0) << "Multiplication by scalars is not allowed.\n";
  CHECK_NE(b_ndim, 0) << "Multiplication by scalars is not allowed.\n";
  ...
  if (a_ndim == 1 && b_ndim == 1) {
    ...
    for (int p = ndim - 3, pa = a_ndim - 3, pb = b_ndim - 3; p >= 0; --p, --pa, --pb) {
      if (pa >= 0 && pb >= 0) {
        if (a_shape[pa] == 1) {
          ...
          CHECK_EQ(a_shape[pa], b_shape[pb]) << "Could not be broadcast.\n";
          out_shape[p] = b_shape[pb];
        }
      } else if (pa >= 0) {
        out_shape[p] = a_shape[pa];
