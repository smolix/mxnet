...
void NumpyUniqueCPUImpl(const NumpyUniqueParam& param,
  ...
  MSHADOW_TYPE_SWITCH(outputs[0].dtype(), DType, {
    ...
    Tensor<cpu, 3, DType> input_tensor(
        workspace.dptr_,
    ...
    std::iota(perm.begin(), perm.end(), 0);
    std::stable_sort(perm.begin(), perm.end(), [&](dim_t a, dim_t b) -> bool {
    ...
    Tensor<cpu, 3, DType> temp_tensor(
        workspace.dptr_, Shape3(valid_num, temp_shape[1], temp_shape[2]), stream);
    // launch kernal to obtain unique array, reuse boolean_mask kernel
    mxnet_op::Kernel<BooleanMaskForwardCPUKernel, cpu>::Launch(
        stream, temp_shape[0], temp_tensor.dptr_, aux.dptr_, prefix_sum.data(), numel);
