...
void NumpyUniqueGPUNoneAxisImpl(const NumpyUniqueParam& param,
  ...
  MXNET_NO_FLOAT16_TYPE_SWITCH(outputs[0].dtype(), DType, {
    ...
    thrust::stable_sort(policy, perm.begin(), perm.end(), argsort_functor1d<DType>(input_data));
    // sorted data in aux
    thrust::device_vector<DType> aux(input_size);
    mxnet_op::Kernel<UniqueComputeAuxGPUKernel, gpu>::Launch(stream,
                                                             input_size,
                                                             thrust::raw_pointer_cast(aux.data()),
                                                             input_data,
                                                             thrust::raw_pointer_cast(perm.data()),
                                                             1);
    // calculate unique mask
    thrust::device_vector<dim_t> mask(input_size);
    mxnet_op::Kernel<UniqueComputeMaskGPUKernel, gpu>::Launch(stream,
                                                              input_size,
    ...
    int32_t valid_num = 0;
    CUDA_CALL(cudaMemcpyAsync(&valid_num,
                              thrust::raw_pointer_cast(&prefix_sum[input_size - 1]),
                              sizeof(int32_t),
    ...
    if (param.return_counts) {
      ...
      *iter = input_size;
      ...
      mxnet_op::Kernel<UniqueReturnCountsKernel, gpu>::Launch(
          stream, valid_num, unique_counts, thrust::raw_pointer_cast(idx.data()));
    }
  });
}
...
void NumpyUniqueGPUImpl(const NumpyUniqueParam& param,
  ...
  MXNET_NO_FLOAT16_TYPE_SWITCH(outputs[0].dtype(), DType, {
    ...
    Tensor<gpu, 3, DType> input_tensor(
        workspace.dptr_,
    ...
    thrust::sequence(policy, perm.begin(), perm.end());
    thrust::stable_sort(
        policy, perm.begin(), perm.end(), argsort_functor2d<DType>(input_data, numel));
    // sorted data in aux
    Tensor<gpu, 2, DType> aux(workspace.dptr_ + input_tensor_3d.shape_.Size(),
                              Shape2(temp_shape[0], temp_shape[1] * temp_shape[2]),
                              stream);
    mxnet_op::Kernel<UniqueComputeAuxGPUKernel, gpu>::Launch(stream,
                                                             temp_shape.Size(),
                                                             aux.dptr_,
    ...
    int32_t valid_num = 0;
    CUDA_CALL(cudaMemcpyAsync(&valid_num,
                              thrust::raw_pointer_cast(&prefix_sum[temp_shape[0] - 1]),
                              sizeof(int32_t),
                              cudaMemcpyDeviceToHost,
                              cuda_stream));
    CUDA_CALL(cudaStreamSynchronize(cuda_stream));
    // store the temp output data, reuse the space of 'input_tensor'
    Tensor<gpu, 3, DType> temp_tensor(
        workspace.dptr_, Shape3(valid_num, temp_shape[1], temp_shape[2]), stream);
    // launch kernal to obtain unique array, reuse boolean_mask kernel
    mxnet_op::Kernel<BooleanMaskForwardKernel, gpu>::Launch(
        stream,
        temp_shape.Size(),
    ...
    Tensor<gpu, 3, DType> output_tensor(
...
NNVM_REGISTER_OP(_npi_unique).set_attr<FComputeEx>("FComputeEx<gpu>", NumpyUniqueGPUForward);
