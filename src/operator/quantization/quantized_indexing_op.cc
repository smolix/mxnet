...
inline bool QuantizedEmbeddingOpShape(const nnvm::NodeAttrs& attrs,
  ...
  SHAPE_ASSIGN_CHECK(
      *in_attrs, quantized_embedding::kWeight, Shape2(param.input_dim, param.output_dim));
...
inline bool QuantizedEmbeddingOpForwardStorageType(const nnvm::NodeAttrs& attrs,
  ...
  if (!dispatched && data_stype == kDefaultStorage && weight_stype == kDefaultStorage) {
    ...
    dispatched = storage_type_assign(
        &out_stype_min, kDefaultStorage, dispatch_mode, DispatchMode::kFCompute);
    dispatched = storage_type_assign(
        &out_stype_max, kDefaultStorage, dispatch_mode, DispatchMode::kFCompute);
  }
...
void QuantizedEmbeddingOpForward(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ(inputs[quantized_embedding::kWeight].ndim(), 2U)
      ...
      << inputs[quantized_embedding::kWeight].ndim() << " dimensional input is given instead";
  mshadow::Stream<cpu>* s = ctx.get_stream<cpu>();
  EmbeddingOpForwardDnsImpl<cpu>(s,
                                 inputs[quantized_embedding::kData],
