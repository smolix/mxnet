...
inline void L1_SVM(const DType& margin,
                   ...
                   const Tensor<gpu, 1, DType>& label,
                   const Tensor<gpu, 2, DType>& src) {
  dim3 dimBlock(mshadow::cuda_impl::kBaseThreadNum);
  dim3 dimGrid(dst.size(0));
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  L1_SVMKernel<mshadow::cuda_impl::kBaseThreadBits, DType>
      <<<dimGrid, dimBlock, 0, stream>>>(margin, reg_coef, dst, label, src);
...
inline void L2_SVM(const DType& margin,
  ...
  dim3 dimGrid(dst.size(0));
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  L2_SVMKernel<mshadow::cuda_impl::kBaseThreadBits, DType>
      <<<dimGrid, dimBlock, 0, stream>>>(margin, reg_coef, dst, label, src);
...
Operator* CreateOp<gpu>(SVMOutputParam param, int dtype) {
  Operator* op = nullptr;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, { op = new SVMOutputOp<gpu, DType>(param); })
  return op;
}
