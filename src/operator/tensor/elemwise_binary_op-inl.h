...
void ElemwiseBinaryOp::RspRspOp(mshadow::Stream<cpu>* s,
  ...
  MSHADOW_IDX_TYPE_SWITCH(rsp.aux_type(rowsparse::kIdx), IType, {
    MSHADOW_TYPE_SWITCH(output.dtype(), DType, {
      ...
      const size_t num_rows_l =
          lhs_is_dense ? lhs.shape()[0] : lhs.aux_shape(rowsparse::kIdx).Size();
      const size_t num_rows_r =
          rhs_is_dense ? rhs.shape()[0] : rhs.aux_shape(rowsparse::kIdx).Size();
      if (is_dense_result) {
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    if (i < num_rows) {
      for (int j = csr_indptr[i]; j < csr_indptr[i + 1]; ++j) {
        KERNEL_ASSIGN(out[i * num_cols + csr_indices[j]],
                      req,
  ...
  MSHADOW_XINLINE static void Map(int tid,
    ...
    if (tid < WARP_SIZE * num_rows) {
      ...
      for (int j = csr_indptr[row_id] + warp_id; j < csr_indptr[row_id + 1]; j += WARP_SIZE) {
        KERNEL_ASSIGN(out[row_id * num_cols + csr_indices[j]],
                      req,
        ...
        MSHADOW_IDX_TYPE_SWITCH(csr_indptr.type_flag_, CType, {
          MXNET_ASSIGN_REQ_SWITCH(req, Req, {
            ...
            Copy(output.aux_data(kIdx).FlatTo1D<xpu, IType>(),
                 csr.aux_data(kIdx).FlatTo1D<xpu, IType>(),
                 s);
            Copy(output.aux_data(kIndPtr).FlatTo1D<xpu, CType>(),
                 csr.aux_data(kIndPtr).FlatTo1D<xpu, CType>(),
                 s);
          });
        });
      });
...
struct ElemwiseDnsRspDnsKernel {
  ...
  MSHADOW_XINLINE static void Map(int i,
                                  DType* out,
                                  DType* dns_data,
                                  const DType* rsp_data,
                                  const IType* rsp_indices,
                                  const nnvm::dim_t num_rows,
    ...
    if (i < nz_rows * num_cols) {
      ...
      KERNEL_ASSIGN(
          out[dns_row * num_cols + col],
          req,
          OP::Map(dns_data[dns_row * num_cols + col], rsp_data[rsp_idx * num_cols + col]));
    }
  }
};
...
void ElemwiseBinaryOp::DnsRspDnsOp(mshadow::Stream<xpu>* s,
  ...
  const bool supported_op =
      std::is_same<OP, mshadow_op::minus>::value || std::is_same<OP, mshadow_op::plus>::value;
  CHECK(supported_op == true) << "Only plus and minus supported now for elemwise operation between "
                                 "default and rsp matrices";
