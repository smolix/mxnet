  ...
  MSHADOW_TYPE_SWITCH(lhs.data().type_flag_, DType, {
    MSHADOW_IDX_TYPE_SWITCH(lhs.aux_data(kIdx).type_flag_, IType, {
      if (lhs.storage_initialized() && rhs.storage_initialized()) {
        ...
        CUDA_CALL(cudaMemcpyAsync(&nnr_out,
                                  &common_row_table[num_rows - 1],
                                  sizeof(nnvm::dim_t),
        ...
        Kernel<RspElemwiseKernel<mshadow_op::plus>, gpu>::Launch(s,
                                                                 lhs_nz_rows * num_cols,
                                                                 output.data().dptr<DType>(),
                                                                 common_row_table,
                                                                 lhs.data().dptr<DType>(),
                                                                 lhs_indices.dptr<IType>(),
                                                                 lhs_nz_rows,
                                                                 num_cols);
        Kernel<RspElemwiseKernel<OP>, gpu>::Launch(s,
                                                   rhs_nz_rows * num_cols,
                                                   output.data().dptr<DType>(),
                                                   common_row_table,
                                                   rhs.data().dptr<DType>(),
                                                   rhs_indices.dptr<IType>(),
                                                   rhs_nz_rows,
                                                   num_cols);
      } else {
        if (lhs.storage_initialized()) {
          if (req == kWriteTo) {
            ...
            Copy(output.data().FlatTo1D<gpu, DType>(), lhs.data().FlatTo1D<gpu, DType>(), s);
            Copy(output.aux_data(kIdx).FlatTo1D<gpu, IType>(),
                 lhs.aux_data(kIdx).FlatTo1D<gpu, IType>(),
                 s);
          ...
          if (std::is_same<OP, mshadow_op::minus>::value) {
            ...
            Copy(output.data().FlatTo1D<gpu, DType>(), rhs.data().FlatTo1D<gpu, DType>(), s);
          }
          if (req == kWriteTo) {
            Copy(output.aux_data(kIdx).FlatTo1D<gpu, IType>(),
                 rhs.aux_data(kIdx).FlatTo1D<gpu, IType>(),
                 s);
          }
        } else {
...
void ElemwiseBinaryOp::DnsCsrDnsOp(mshadow::Stream<gpu>* s,
  ...
  const bool supported_op =
      std::is_same<OP, mshadow_op::minus>::value || std::is_same<OP, mshadow_op::plus>::value;
...
NNVM_REGISTER_OP(elemwise_add)
    ...
    .set_attr<FComputeEx>("FComputeEx<gpu>",
                          ElemwiseBinaryOp::ComputeEx<gpu, op::mshadow_op::plus>);

NNVM_REGISTER_OP(_grad_add).set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCCompute{"add"});

NNVM_REGISTER_OP(_backward_add)
    .set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCBwdUseNone{"identity", "identity"});

NNVM_REGISTER_OP(elemwise_sub)
    .set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCCompute{"sub"})
    .set_attr<FComputeEx>("FComputeEx<gpu>",
                          ElemwiseBinaryOp::ComputeEx<gpu, op::mshadow_op::minus>);

NNVM_REGISTER_OP(_backward_sub)
    .set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCBwdUseNone{"identity", "negation"});
...
NNVM_REGISTER_OP(_backward_mul)
    .set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCBwdUseIn{"right", "left"});

NNVM_REGISTER_OP(elemwise_div).set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCCompute{"div"});

NNVM_REGISTER_OP(_backward_div)
    .set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCBwdUseIn{"div_grad", "div_rgrad"});

NNVM_REGISTER_OP(_mod).set_attr<FCompute>("FCompute<gpu>", ElemwiseBinaryRTCCompute{"mod"});
