...
inline bool RavelOpShape(const nnvm::NodeAttrs& attrs,
  ...
  if ((*in_attrs)[0].ndim() > 0) {
    CHECK_EQ((*in_attrs)[0].ndim(), 2) << "Input to ravel operator must be two-dimensional.";
...
inline bool UnravelOpShape(const nnvm::NodeAttrs& attrs,
  ...
  if ((*out_attrs)[0].ndim() > 0) {
    ...
    CHECK_EQ((*out_attrs)[0].ndim(), 2) << "Output of unravel operator must be two-dimensional.";
    CHECK_EQ((*out_attrs)[0][0], shape.ndim())
        << "First dimension of output of ravel operator does not match shape parameter dimension.";
    SHAPE_ASSIGN_CHECK(
        *in_attrs, 0, mxnet::TShape(out_shape.data() + 1, out_shape.data() + out_shape.ndim()));
    return true;
  }
  return false;
}
...
void RavelForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, OType, {
    ...
    mxnet_op::Kernel<ravel_index, xpu>::Launch(
        s, out.size(0), out.size(0), in.size(0) / out.size(0), work.dptr_, in.dptr_, out.dptr_);
  });
}
