...
inline bool SparseRetainOpShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ(in_attrs->size(), 2U) << "sparse_retain operator takes 2 arguments (" << in_attrs->size()
                                 << " given)";
      ...
      MSHADOW_TYPE_SWITCH(idx_data.type_flag_, IType, {      // index array data type
        if (input_idx.Size() == static_cast<size_t>(input_nd.shape()[0])) {  // input rsp is dense
          ...
          if (output_idx.type_flag_ == idx_data.type_flag_) {  // same type, use Copy
            ...
            Kernel<SparseRetainCopyIndices, xpu>::Launch(
                s, num_rows_retained, output_idx.dptr<RType>(), idx_data.dptr<IType>());
          }
          // copy data
          if (std::is_same<xpu, cpu>::value) {  // For cpu, parallelize by rows
            Kernel<SparseRetainCopyRetainedRowsFromDnsPerRow, xpu>::Launch(
                s,
                idx_data.Size(),
                output_data.dptr<DType>(),
                input_data.dptr<DType>(),
                idx_data.dptr<IType>(),
                row_length);
          } else {  // For gpu, parallelize by elements
            Kernel<SparseRetainCopyRetainedRowsFromDnsPerElem, xpu>::Launch(
                s,
                output_data.Size(),
                output_data.dptr<DType>(),
                input_data.dptr<DType>(),
          ...
          Kernel<SparseRetainRspThreadKernel, xpu>::Launch(s,
                                                           idx_data.Size(),
                                                           output_data.dptr<DType>(),
                                                           output_idx.dptr<RType>(),
                                                           input_data.dptr<DType>(),
                                                           input_idx.dptr<RType>(),
                                                           idx_data.dptr<IType>(),
                                                           input_data.shape_[0],
                                                           row_length);
        }
...
void SparseRetainOpBackwardEx(const nnvm::NodeAttrs& attrs,
  ...
  if (req[sr::kArr] == kNullOp)
    return;
  CHECK_EQ(req[sr::kArr], kWriteTo);

  CHECK_EQ(inputs.size(), 2U);
  CHECK_EQ(outputs.size(), 2U) << "sparse_retain does not support calculating gradients of indices";
