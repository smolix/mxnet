...
def test_np_array_creation():
    dtypes = [_np.int8, _np.int32, _np.float16, _np.float32, _np.float64, _np.bool_, _np.bool_,
...
def test_np_zeros():
    ...
    for shape in shapes:
        for dtype in [_np.bool_, bool, _np.bool_, 'bool']:
            check_zero_array_creation(shape, dtype)
...
def test_np_ndarray_astype():
    ...
    dtypes = [np.int8, np.uint8, np.int32, np.float16, np.float32, np.float64, np.bool_, np.bool_,
...
def test_boolean_index_assign():
    ...
    mx_mask = np.array([[False,True], [True,False], [True,False]],dtype=np.bool_)
    np_data = mx_data.asnumpy()
