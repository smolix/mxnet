...
@pytest.mark.parametrize('dtype', [np.int8, np.uint8, np.int32, np.int64, np.float16, np.float32, np.float64, np.bool_])
...
def test_npi_boolean_assign():
    ...
    for hybridize in [False]:
        for config in configs:
            ...
            valid_num = 0
            while valid_num == 0:
                mx_mask = np.random.choice(np.array([False, True], dtype=np.bool_), size=mshape)
                if test_data.size == 0:
                    break
...
def test_np_mixed_precision_binary_funcs():
    itypes = [np.bool_, np.int8, np.int32, np.int64]
...
def test_np_concat():
    ...
    dtypes = [np.float32, np.float64, np.bool_]
...
def test_np_true_divide():
    ...
    dtypes = [np.bool_, np.int8, np.uint8, np.int32, np.int64, np.float16, np.float32, np.float64]
    itypes = [np.bool_, np.int8, np.uint8, np.int32, np.int64]
    ftypes = [np.float16, np.float32, np.float64]
    for shape_pair, dtype in itertools.product(shapes, dtypes):
        ...
        out_mx = a / b
        if onp.issubdtype(dtype, onp.integer) or (dtype is np.bool_):
...
def test_np_quantile():
    ...
    for hybridize, keepdims, q_scalar, (a_shape, q_shape, axis), interpolation, dtype in \
        ...
        # float16: non-linear interpolation (nearest/lower/higher/midpoint) selects
        # and averages values entirely in fp16.  When bracket values straddle an fp16
        # exponent boundary the midpoint rounds to a different mantissa block,
        # producing errors up to ~256 ULPs (~0.13 relative).  Loosen for fp16 only.
...
def test_np_unary_bool_funcs():
    def check_unary_func(func):
        ...
        for [hybridize, dtype, src] in itertools.product(hybridize_list, dtype_list, src_list):
            ...
            # if `out` is given and dtype == np.bool_
            mx_x = np.ones_like(mx_data).astype(np.bool_)
            np_x = mx_x.asnumpy()
